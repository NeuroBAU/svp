...
7. **Bare imports for delivered scripts.** Scripts delivered to `svp/scripts/` in the repository must use bare imports for inter-script references (e.g., `from pipeline_state import load_state`), NOT package imports (e.g., `from svp.scripts.pipeline_state import load_state`). This is because the launcher copies these scripts into project workspaces where they run with `PYTHONPATH=scripts` and bare imports (see Section 6.2, dual-file synchronization contract). Package-style `svp.scripts.X` imports resolve only when the SVP package is installed — they fail in project workspaces. The `svp.scripts.X` form is used ONLY in `pyproject.toml` entry points, which reference the installed package.
8. **CLI entry point guards.** Every script delivered to `svp/scripts/` that is invoked directly via `python scripts/X.py` must include an `if __name__ == "__main__"` guard that calls its entry-point function. Without this guard, `python scripts/X.py` imports and defines functions but executes nothing.
9. **Runtime completeness.** Each delivered script must contain all functions that other delivered scripts import from it. During the workspace development cycle, `scripts/` copies may accumulate additional orchestration functions (e.g., `run_infrastructure_setup` in `dependency_extractor.py`) beyond what the canonical `src/unit_N/stub.py` contains. The delivered version must include these functions. When a workspace `scripts/` file has functions that the `src/unit_N/stub.py` does not, the Git Repo Agent must include the additional functions in the delivered file.
10. **CLI wrappers must delegate, not reimplement.** Three units produce both a library module and CLI wrapper scripts: Unit 6 (`stub_generator.py` + `generate_stubs.py`), Unit 7 (`dependency_extractor.py` + `setup_infrastructure.py`), and Unit 10 (`routing.py` + `update_state.py` + `run_tests.py`). The CLI wrapper scripts must be thin wrappers that import and call the entry-point functions from their library module (e.g., `update_state.py` imports `update_state_main()` from `routing`, `run_tests.py` imports `run_tests_main()` from `routing`). They must NOT reimplement dispatch logic, test execution, or orchestration inline. Reimplemented wrappers drift from the canonical implementation silently -- they are not covered by the scripts synchronization check and diverge in behavior from the tested code.
11. **Cross-unit CLI contracts.** Unit 10 (routing script) generates PREPARE and POST command strings that are executed as shell commands against other scripts. The argument syntax in these generated commands constitutes a cross-unit contract: the receiving script must accept every argument that Unit 10 generates. Specifically, `prepare_task.py` (Unit 9) must accept `--output` to override the output path, and `update_state.py` (Unit 10's own wrapper) must accept `--gate` for gate response dispatch. When adding or changing arguments in Unit 10's generated command strings, the receiving script's CLI interface must be updated in the same commit.

This mapping is critical for the delivered software to function. A repository that passes tests in the workspace but uses workspace-internal paths (`src.unit_N.stub`) in its entry points or imports will fail when installed by the end user. Equally critical: a repository that uses package imports (`svp.scripts.X`) in scripts that will be copied to project workspaces will fail at runtime.
