...
**Group B commands (help, hint, ref, redo, bug, oracle):** include complete action cycle: (1) run `prepare_task.py --agent <type> --project-root .`, (2) spawn agent, (3) write terminal status to `last_status.txt`, (4) run `update_state.py --phase <phase>`, (5) re-run routing script.
...
**Invariant (Bug S3-79):** Group B slash command skill definitions must be thin state-transition triggers. They enter the relevant session state and redirect to the routing script. They must never contain instructional text for building user-facing content. Content construction is the routing script's responsibility (P21, P23).
