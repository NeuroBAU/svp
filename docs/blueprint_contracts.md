...
- After write, reads file bytes back and computes SHA-256 hex digest. Stores in `state.state_hash`.
- Re-writes file with updated `state_hash`.
...
TEST_OUTPUT_PARSERS: Dict[str, Callable[[str, str, int, Dict[str, Any]], "TestResult"]]
...
- Key `"python"`: parses pytest stdout for `N passed, M failed, K errors`. Returns `TestResult(status, passed, failed, errors, output, collection_error)`. Collection errors detected by `collection_error_indicators` from registry.
- Key `"r"`: parses testthat output for `OK:`, `Failed:`, `Warnings:` counts. Maps `OK` -> passed, `Failed` -> failed, `Warnings` -> errors.
- Key `"plugin_markdown"`: parses markdown lint output (e.g., markdownlint). Maps lint errors to `TESTS_FAILED`, clean output to `TESTS_PASSED`. Returns `TestResult` with simplified counts.
- Key `"plugin_bash"`: parses `bash -n` syntax check output. Syntax errors map to `TESTS_FAILED`, clean output to `TESTS_PASSED`. Returns `TestResult`.
- Key `"plugin_json"`: parses JSON validation output (e.g., `python -m json.tool`). Parse errors map to `TESTS_FAILED`, valid JSON to `TESTS_PASSED`. Returns `TestResult`.
- If parsing fails: returns `TestResult(status="TESTS_ERROR", ...)` with raw output.
