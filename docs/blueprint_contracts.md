...
- Each violation dict has fields: `check` (one of `"dag"`, `"reachability"`, `"phantom_call"`, `"calls_resolution"`, `"blueprint_missing"`), `severity` (`"error"` or `"warning"`), `location` (unit name or file path), `description` (1-line human-readable string).
