...
**`/svp:oracle` command:** includes test project selection UX. The routing script builds the complete numbered list deterministically (hardcoded F-mode "SVP Pipeline" as item 1 + auto-discovered E-mode from `examples/`), includes a number-to-path mapping and a `post` command (`update_state.py --command oracle_test_project_selection`). The orchestrator presents the list verbatim, writes the selected path to `last_status.txt`, and runs the POST command to persist the selection. **(Bug S3-76 + S3-77 fix.)**
