...
**run_tests_main toolchain key mismatch (NEW IN 2.2 -- Bug S3-100).** `run_tests_main` must read the test command from `toolchain["testing"]["run_command"]` (not `toolchain["test"]["command"]`), resolve `run_prefix` from `toolchain["environment"]["run_prefix"]` (not top-level `toolchain["run_prefix"]`), and normalize the `{test_path}` placeholder to `{target}` before calling `resolve_command`. The integration test fixture `_create_toolchain_file()` must use the same key structure as the production toolchain JSON files.
**helper-svp.R path resolution implementation (NEW IN 2.2 -- Bug S3-101).** The `helper-svp.R` template generated by infrastructure setup must use `testthat::test_path()` navigation to locate the project root, as specified by Bug S3-48. The implementation must be `source(file.path(testthat::test_path(), "..", "..", unit_file), local = parent.frame())`, not `source(file.path("R", unit_file), ...)` which resolves relative to CWD.
**Sync protocol redesign: one-way sync, flat test imports, docs consolidation (NEW IN 2.2 -- Bug S3-103).** The workspace is the single source of truth; the repo is a derived artifact. `sync_workspace.sh` performs one-way sync (workspace → repo) with safety warnings if repo files are newer. Tests must use flat module imports (`from module_name import ...`), not stub imports (`from src.unit_N.stub import ...`). Repo paths are stored in `.svp/sync_config.json`. All documentation is consolidated in `repo/docs/` — no `specs/`, `blueprint/`, `references/`, `CLAUDE.md`, or `project_context.md` at repo root. `restore_project()` auto-discovers artifacts from the repo's `docs/` directory.
**Oracle task prompt must embed test project artifacts (NEW IN 2.2 -- Bug S3-102).** `_prepare_oracle_agent` in `prepare_task.py` must embed the test project's `oracle_manifest.json`, `project_context.md`, `stakeholder_spec.md`, `blueprint_prose.md`, and `blueprint_contracts.md` as sections in the oracle task prompt during both dry_run and green_run phases. For E-mode, artifacts are read from `examples/<project_name>/`. For F-mode, artifacts are read from `docs/`. The oracle must not self-load these files — they must arrive pre-assembled in the task prompt per the verbatim relay principle.
...
**`ensure_project_settings()` contract.** The launcher's helper that writes the file must satisfy six properties, all load-bearing:
...
3. **Self-healing.** If the user moves the SVP repo on disk (to a different path, to a new machine, to a different drive), re-running the helper with the new `plugin_root` rewrites the stale `extraKnownMarketplaces.svp.source.path`. No manual editing of the settings file is required. **(Widened by Bug S3-127.)** The self-heal also triggers when the stored path no longer contains `.claude-plugin/marketplace.json` — this catches cache-parent paths that were written before the invariant was enforced.
...
6. **Marketplace-root validated (NEW IN 2.2 — Bug S3-127).** The path written into `extraKnownMarketplaces.svp.source.path` must resolve to a real directory containing `.claude-plugin/marketplace.json`. `plugin_root.parent` is no longer sufficient on its own because Claude Code's internal plugin cache layout (`~/.claude/plugins/cache/<marketplace>/<plugin>/<version>/`) has a parent directory that is NOT a marketplace root. The helper consults `_find_marketplace_root(plugin_root)`, which resolves the marketplace root via (a) `SVP_MARKETPLACE_ROOT` env var, (b) `plugin_root.parent` if and only if it contains `marketplace.json`, or (c) walking up from the launcher's own `__file__` location. If no valid marketplace root is reachable, the helper raises `FileNotFoundError` with an actionable message instructing the user to set `SVP_MARKETPLACE_ROOT`. Silent fallbacks are prohibited — a missing marketplace root is a loud failure, not a broken settings file.
...
**Unit heading grammar invariant (CHANGED IN 2.2 — Bug S3-116).** Every `## Unit N` heading in `blueprint_prose.md` and `blueprint_contracts.md` MUST use a colon separator followed by the unit name: `## Unit N: <Name>`. The framework's parsers (`_count_unit_headings` in Unit 11 and `_UNIT_HEADING_RE` in Units 8 and 9) all enforce the strict regex `^## Unit (\d+):`. Headings using em-dash (`—`), en-dash (`–`), hyphen (`-`), period (`.`), or any other separator are rejected with a zero-match result. This is the canonical grammar — do not loosen it. **Two enforcement points (Bug S3-116):** (1) `dispatch_agent_status` for `blueprint_author + BLUEPRINT_DRAFT_COMPLETE` and `BLUEPRINT_REVISION_COMPLETE` in Unit 14 invokes the shared deterministic validator `validate_unit_heading_format` from Unit 8 after every blueprint_author completion; if the blueprint contains violations, the dispatch raises `ValueError` with a near-miss diagnostic BEFORE Gate 2.1 is presented. (2) `run_infrastructure_setup` Step 5 in Unit 11 invokes the same validator as a safety net when `_count_unit_headings` returns 0. Both enforcement points call the same Unit 8 function, so the two sides cannot drift. A regression test exercises both the canonical and em-dash paths.
...
3. **Always use flat module imports.** Test files must use `from module_name import ...` (e.g., `from routing import run_tests_main`, `from hooks import HOOKS_JSON_SCHEMA`). Never use `from src.unit_N.stub import ...` — stub paths are workspace-internal and do not exist in the delivered repo. The flat module names match the deployed scripts and are resolved via `pyproject.toml pythonpath = ["scripts"]`. (Bug S3-9, CHANGED IN 2.2 — Bug S3-103)
...
**Delivered repo path invariant (CHANGED IN 2.2 — Bug S3-112).** The delivered repo path is deterministically `{project_root.parent}/{project_name}-repo` (absolute), where `project_name` comes from `profile["name"]` (or `profile["project_name"]`, or `project_root.name` as fallback). `state.delivered_repo_path` MUST be set by `dispatch_agent_status` for `git_repo_agent + REPO_ASSEMBLY_COMPLETE`, not by the agent itself. The agent MUST call the language-appropriate `assemble_*_project()` helper from the `generate_assembly_map` module (Python/R/plugin/mixed) — these helpers already compute and use the canonical path internally. The agent MUST NOT choose a different destination (e.g. `./delivered/`) and MUST NOT directly edit `.svp/pipeline_state.json`. The POST dispatch step computes the canonical path independently, verifies the directory exists, and stores the resolved absolute path on state — if the canonical directory does not exist after `REPO_ASSEMBLY_COMPLETE`, the dispatch raises `ValueError` rather than advancing. A regression test (`TestDeliveredRepoPathInvariant`) enforces this invariant end-to-end against the committed workspace state.
...
**CORRECTION (S3-121).** The diagnosis in this entry was inverted. Claude Code derives skill names from the directory path (`skills/orchestration/` → `svp:orchestration`), not from the frontmatter `name` field. The edit to set frontmatter `name: svp:svp_orchestration` was functionally inert — the real bug was the command filenames loading as `svp:svp_bug` etc. due to the plugin namespace being prepended to already-prefixed filenames. See Section 24.134 for the correct diagnosis and fix.
...
### 24.113 run_tests_main Wrong Toolchain Keys (Post-delivery — Bug S3-100, NEW IN 2.2)

**run_tests_main reads wrong toolchain keys, causing ALL Stage 3 test execution to return TESTS_ERROR (NEW IN 2.2 -- Bug S3-100).** `run_tests_main` read `toolchain.get("test", {}).get("command", "")` but both production toolchain files (`python_conda_pytest.json` and `r_renv_testthat.json`) use the key path `toolchain["testing"]["run_command"]`. Because `toolchain.get("test", {})` returns `{}`, `test_cmd_template` is always empty, triggering the early `TESTS_ERROR` return. Additionally, `run_prefix` was read from `toolchain.get("run_prefix", "")` (top-level) instead of `toolchain["environment"]["run_prefix"]`. The production template also uses `{test_path}` as the placeholder but `resolve_command` only substitutes `{target}`, so the placeholder was never resolved. The integration test fixture `_create_toolchain_file()` used `"test": {"command": "...{target}..."}` matching the buggy code rather than the production schema, masking the failure. Fix: (1) Read `toolchain["testing"]["run_command"]`. (2) Read `toolchain["environment"]["run_prefix"]`. (3) Normalize `{test_path}` to `{target}` before calling `resolve_command`. (4) Update integration test fixture to match production schema.

**Pattern:** P3 (Test Fixture Structural Mismatch). The test fixture used a different JSON schema than production toolchain files. Tests passed because the buggy code matched the buggy fixture. **Detection:** Oracle E-mode run 11 with `examples/gol-r/`.

### 24.114 helper-svp.R Wrong Path Resolution Implementation (Post-delivery — Bug S3-101, NEW IN 2.2)

**helper-svp.R uses wrong path resolution despite correct spec (NEW IN 2.2 -- Bug S3-101).** The `helper-svp.R` template generated by `infrastructure_setup.py` used `source(file.path("R", unit_file), local = parent.frame())` which resolves relative to CWD. During testthat execution, CWD is `tests/testthat/`, so `svp_source("unit_1.R")` resolves to `tests/testthat/R/unit_1.R` — wrong. The spec (Bug S3-48) explicitly requires `testthat::test_path()` navigation: `source(file.path(testthat::test_path(), "..", "..", unit_file), local = parent.frame())`. The blueprint contracts for Unit 11 did not mention helper-svp.R generation at all — complete omission. Existing regression tests for S3-48 verified only that the function existed in source code, not that the generated content was correct. Fix: (1) Update template to use `testthat::test_path()` navigation. (2) Add blueprint contract for helper-svp.R. (3) Add content-verification regression test.

**Pattern:** P7 (Blueprint Omission). The spec requirement was correct but the blueprint had no contract for this file's generation, so the implementation agent had no specification to follow. **Detection:** Oracle E-mode run 11 with `examples/gol-r/`.

### 24.115 Oracle Task Prompt Missing Test Project Artifacts (Post-delivery — Bug S3-102, NEW IN 2.2)

**Oracle task prompt embeds only test project path, not artifact contents (NEW IN 2.2 -- Bug S3-102).** `_prepare_oracle_agent` in `prepare_task.py` (Unit 13) appended only the test project directory path string to the oracle task prompt, not the actual artifact file contents. The spec (Section 35.10, items 7-7a) explicitly lists test project artifacts (`oracle_manifest.json`, `project_context.md`, `stakeholder_spec.md`, blueprint documents) as required oracle inputs. The blueprint contract (line 987) mentions "test project artifacts" but the code never implemented the loading. The oracle agent compensated by reading files from disk independently, wasting tool calls and violating the verbatim task prompt relay principle. Fix: (1) After appending the test project path, read and embed all 5 artifact files. E-mode reads from `examples/<project>/`, F-mode reads from `docs/`. (2) Embed artifacts in both dry_run and green_run phases.

**Pattern:** P7 (Specification Omission at Implementation Level). The spec input list was explicit, the blueprint contract mentioned it, but the code never implemented the artifact loading. **Detection:** Oracle E-mode runs 11-13, all flagged as carry-over WARN.

### 24.116 Sync Protocol Redesign: One-Way Sync, Flat Imports, Docs Consolidation (Post-delivery — Bug S3-103, NEW IN 2.2)

**Workspace-repo sync was bidirectional with mtime-based conflict resolution, tests used workspace-internal stub imports, and documentation was duplicated across scattered directories (NEW IN 2.2 -- Bug S3-103).** Multiple issues addressed in a single protocol redesign: (1) `sync_workspace.sh` used bidirectional `sync_pair()` with mtime comparison, allowing repo edits to accidentally overwrite workspace changes. Fix: one-way sync (workspace → repo) with safety warning if repo is newer. (2) Tests used `from src.unit_N.stub import ...` which only works in workspace (requires `src/` on pythonpath), not in a standalone delivered repo. Fix: standardize on flat module imports (`from module_name import ...`) resolved via `pythonpath = ["scripts"]`. Updated spec rule P3, TEST_AGENT_DEFINITION, and all 46 existing test files. (3) Repo had documentation duplicated across `specs/`, `blueprint/`, `references/`, root, AND `docs/`. Fix: consolidate all docs into `docs/` only; `CLAUDE.md` and `project_context.md` also moved from repo root to `docs/` (they are restore-only artifacts, not functional at repo root). (4) `restore_project()` required explicit CLI args for every artifact path. Fix: auto-discovers from repo's `docs/` directory structure. (5) Repo paths were hardcoded in `sync_workspace.sh`. Fix: stored in `.svp/sync_config.json`, written by `restore_project()`.
...
### 24.121 Hook Scripts Not Deployed to .claude/scripts/ (Post-delivery — Bug S3-108, NEW IN 2.2)

The plugin `hooks.json` references `.claude/scripts/write_authorization.sh`, `.claude/scripts/non_svp_protection.sh`, `.claude/scripts/stub_sentinel_check.sh`, and `.claude/scripts/monitoring_reminder.sh`. These scripts are generated by Unit 17 and assembled into `svp/hooks/` during Stage 5. However, neither `create_new_project()`, `restore_project()`, nor `sync_workspace.sh` copies them to `.claude/scripts/` in the project workspace. Hooks fire on every tool use and fail with "No such file or directory" (non-blocking).

**Root cause:** The hook assembly (Unit 23 `assemble_plugin_components()`) writes scripts to `svp/hooks/` in the delivered repo, but the project creation and sync flows never deploy them to the workspace.
...
### 24.125 git_repo_agent Improvised Delivered Repo Destination, Wrote Relative String to State (Post-delivery — Bug S3-112, NEW IN 2.2)

**Reported externally** on 2026-04-12 by a user running SVP 2.2.0 against a Python project `debrief1.0`. After Stage 5 completed with `pipeline_complete`, the delivered repository was written to `./delivered/` inside the project root instead of the canonical sibling `../debrief1.0-repo/`. `.svp/pipeline_state.json` held `"delivered_repo_path": "delivered"` — a bare relative string, not an absolute path. No sibling `debrief1.0-repo/` directory existed. Stage 5 routed to `pipeline_complete` cleanly: `gate_5_1_repo_test` returned TESTS PASSED, `compliance_scan` returned COMMAND_SUCCEEDED. The misplacement did not surface as a gate failure — it was silently inconsistent with the `*-repo` convention.

**Impact:**
- `sync_debug_docs.py` reads `state.delivered_repo_path` and constructs `Path(state.delivered_repo_path)`. A bare `"delivered"` resolves relative to the script's CWD, which is fragile.
- The `bug_triage_agent` is documented to correlate bugs via `repo_to_workspace` in the assembly map, using `delivered_repo_path` as the base — it looks in the wrong place.
- The write-authorization hook's `*-repo/*` allowlist does not match `./delivered/`, so authorized debug edits to delivered code would be rejected.
- The Oracle agent's nested-session flow assumes the sibling layout.
- Downstream failures are silent and deferred: the pipeline reports `pipeline_complete` and the human believes delivery succeeded. Breakage only surfaces later, during debug, oracle, or tooling consumption.

**Root cause — agent discretion on a deterministic fact.** The delivered repo path is fully computable from `(project_root, project_name)` — it is `project_root.parent / f"{project_name}-repo"` by convention, and this is encoded inside `assemble_python_project()` / `assemble_r_project()` / `assemble_plugin_project()` / `assemble_mixed_project()` in `src/unit_23/stub.py`. But the pipeline code did NOT: (a) compute it in the prepare step, (b) inject it into the agent's task prompt as a constraint, (c) compute or validate it in the POST dispatch step, (d) tell the agent which helper function to call by name. The agent was an LLM with no binding; given no constraint, it improvised `./delivered/` and stored the relative string to state. In pass2's own build the agent happened to do it right (the committed state holds the absolute sibling path), but it was pure luck — the code offered zero guarantees.

Specifically:
- `GIT_REPO_AGENT_DEFINITION` (`src/unit_23/stub.py:22-120`) had NO "Delivered Repo Location" section, NO named references to the `assemble_*_project()` helpers, NO prohibition on `./delivered/`, NO instructions about `state.delivered_repo_path`.
- `_prepare_git_repo_agent` (`src/unit_13/stub.py:1106-1141`) loaded profile and assembly map but did not compute or inject the canonical delivered path.
- `dispatch_agent_status` for `git_repo_agent` + `REPO_ASSEMBLY_COMPLETE` (`src/unit_14/stub.py:2567-2570`) was a no-op `_copy(state)` — it neither computed nor validated the delivered path, nor called `set_delivered_repo_path`.

**Fix (Bug S3-112) — defense in depth, three layers:**

1. **Task prompt injection** (`src/unit_13/stub.py::_prepare_git_repo_agent`): compute `canonical_delivered_path = (project_root.parent / f"{profile_name}-repo").resolve()` and inject it into the task prompt as a "Delivered Repo Path (REQUIRED)" section with a directive that the agent MUST place the delivered repo at exactly this path and MUST call the language-appropriate `assemble_*_project()` helper.

2. **Explicit agent binding** (`src/unit_23/stub.py::GIT_REPO_AGENT_DEFINITION`): add a new "## Delivered Repo Location" section that states the `{project_root.parent}/{project_name}-repo` convention, names the four assembler helpers (Python/R/plugin/mixed), forbids `delivered/`/`delivered_repo/`/`output/`/any sub-directory of the project root, and forbids manual edits to `.svp/pipeline_state.json`. Note that `state.delivered_repo_path` is automatically set by the dispatch step.

3. **POST dispatch validation** (`src/unit_14/stub.py::dispatch_agent_status` for `git_repo_agent + REPO_ASSEMBLY_COMPLETE`): load the profile, compute the canonical path independently, verify the directory exists, call `set_delivered_repo_path(state, str(canonical_path.resolve()))`. If the directory does NOT exist, raise `ValueError` with a clear message referencing Bug S3-112 and the expected path. This is the hard guarantee: after every successful dispatch, `state.delivered_repo_path` is always absolute, always sibling, always exists on disk — regardless of what the agent did.

**Why three layers:** Layer 1 (prepare injection) and Layer 2 (agent binding) reduce the probability of Layer 3 having to raise. Layer 3 is the absolute guarantee. Each alone is leaky — a missing task-prompt section can be ignored, an agent definition can be misinterpreted, a validation-only check fires after the damage is done. Together they eliminate the failure mode.
...
- **Operator documentation about direct `last_status.txt` writes (previous item 2): DONE.** A new Section 16 ("Manual State Manipulation") in the orchestration skill (`src/unit_26/stub.py::ORCHESTRATION_SKILL`) documents the canonical three-step sequence when the operator must bypass the normal action cycle: (1) write `last_status.txt`, (2) run `update_state.py --phase --status` explicitly to invoke dispatch, (3) call `routing.py`. The section forbids direct edits to `.svp/pipeline_state.json`, explains why relying on the S3-114 self-heal is wrong, and ends with the summary rule: "`last_status.txt` is the message; `update_state.py` is the delivery. A message without delivery never arrives." The new skill section is regenerated into `svp/skills/orchestration/SKILL.md` via `regenerate_deployed_artifacts()` on sync, so every Claude Code session loads it as part of the orchestration skill on startup.
...
### 24.129 Unit Heading Format Silently Parses to Zero Units (Post-delivery — Bug S3-116, NEW IN 2.2)

**Reported externally** by an operator on a real SVP project (`debrief1.0`). During the Stage 2→3 transition, the operator invoked pre-Stage-3 infrastructure setup and watched it silently compute `total_units = 0`. Instrumenting `_count_unit_headings` revealed the cause: the blueprint used em-dash separators (`## Unit 1 — Plugin Scaffold`) instead of the canonical colon separator (`## Unit 1: Plugin Scaffold`). The framework's strict regex `^## Unit (\d+):` returned zero matches, `_count_unit_headings` returned 0, and downstream code would later fail with the cryptic error `"No unit headings found in blueprint. Cannot proceed with setup."`

**Three layers of root cause:**

1. **No early deterministic validation.** `blueprint_checker` (Unit 19) is an LLM agent. Its checklist did not require unit heading format validation. The blueprint could pass `gate_2_1_blueprint_approval`, `gate_2_2_blueprint_post_review`, and `alignment_check` with em-dash headings intact. The format violation only surfaced at Stage 3 pre-infrastructure setup.

2. **Silent parser output.** `_count_unit_headings` in `src/unit_11/stub.py` and `extract_units` in `src/unit_8/stub.py` (via `_UNIT_HEADING_RE`) both returned empty results for a non-empty blueprint. No warning, no diagnostic, no near-miss reporting.

3. **Weak downstream error.** `run_infrastructure_setup` raised `ValueError("No unit headings found in blueprint. Cannot proceed with setup.")` — a message with zero information about which files were scanned, what format was expected, or what lines were actually found. The operator had to instrument the parser themselves to discover the em-dash.

**The spec had the grammar documented at Section 1949 ("Both files use the same unit heading structure `## Unit N: Name`") but nothing enforced the rule deterministically until the parser silently failed at Stage 3.**

**Fix (Bug S3-116) — deterministic two-enforcement-point validation with a shared function:**

1. **Shared validator in Unit 8** — new public functions `validate_unit_heading_format(blueprint_dir) -> List[Tuple[str, int, str]]` and `format_unit_heading_violations(near_misses, max_shown=10) -> str` in `src/unit_8/stub.py`. The validator scans both blueprint files for lines matching the loose pattern `^## Unit \d+\b` and filters to those NOT matching the strict pattern `^## Unit \d+:`. Returns a list of `(filename, line_number, line_text)` tuples — empty list means the blueprint is format-clean. The formatter produces a human-readable error message including the first 10 violations, the canonical example, spec Section 1949 reference, and Bug S3-116 reference. Both functions are the single source of truth for unit heading format validation.

2. **Writing-side enforcement in `dispatch_agent_status`** — Unit 14's `blueprint_author + BLUEPRINT_DRAFT_COMPLETE` and `BLUEPRINT_REVISION_COMPLETE` branches call `validate_unit_heading_format(project_root / "blueprint")` immediately after the agent completes. If the validator returns any violations, dispatch raises `ValueError` with the shared formatted message. The pipeline halts at dispatch time — BEFORE Gate 2.1 is presented to the human. The operator sees the violation with full near-miss diagnostic as soon as blueprint_author finishes, can re-invoke the agent with the feedback, and does not waste gate cycles approving a malformed blueprint.

3. **Extraction-side enforcement in `run_infrastructure_setup`** — Unit 11's Step 5 calls the same `validate_unit_heading_format` when `_count_unit_headings` returns 0. If violations are found, raises `ValueError` with the shared formatted message. If the validator returns empty (blueprint has no `## Unit N` lines at all), raises a distinct "blueprint may be empty or missing" error. This is the safety net: if the writing-side dispatch check was skipped (operator bypass, dispatch regression), the extraction-side check catches it. The two enforcement points share the same Unit 8 function, so they cannot drift — what the writing side accepts, the extraction side also accepts, and what one rejects, both reject.

4. **Agent definition updates** — `BLUEPRINT_AUTHOR_DEFINITION` and `BLUEPRINT_REVIEWER_DEFINITION` in Unit 20 explicitly reference the unit heading grammar rule AND note that the dispatch step halts the pipeline on violation. This is belt-and-suspenders — LLM agents reading their own role definitions will see the rule as a hard constraint rather than a stylistic preference.

**Pattern:** P-NEW — **"A deterministic parser must never silently return empty for a structured document without reporting what it actually found."** When a regex returns 0 matches on a non-empty input, the parser should either (a) fall back to a looser pattern for diagnostics and surface the near-miss in an error, or (b) raise with file/line context. Silent empty return is the wrong default for any parser operating on a human/LLM-written document. Combined with the lesson from Bug S3-112 ("agent discretion on deterministic facts is a liability"), the rule becomes: **when an LLM agent produces a structured artifact, every grammar rule the parser depends on must be enforced deterministically at the earliest-possible dispatch step.**

**Also: documented grammar requires enforcement infrastructure, not just prose.** Spec Section 1949 documented the `## Unit N: Name` rule all along. Documentation is not enforcement. Bug S3-116 added the missing enforcement infrastructure — the validator + two dispatch points — so the spec rule is now backed by runtime checks.

**Regression tests (19 total):** 11 tests in `tests/unit_8/test_unit_8.py` for the shared validator (canonical, em-dash, en-dash, hyphen, period, mixed, non-unit-heading ignore, filename/line-number accuracy, message formatting, truncation, empty-input). 4 tests in `tests/unit_14/test_unit_14.py` for the dispatch-side enforcement (success, em-dash failure, revision path, unknown status). 3 tests in `tests/unit_11/test_unit_11.py` for the extraction-side safety net (em-dash near-miss, truly-empty path, uses-shared-validator). 1 integration regression test exercising the full happy-then-sad-then-happy flow.

**Rejected alternatives:**
- **Loosen the regex** to accept multiple separators — breaks the documented grammar and creates a moving target as new separator styles emerge.
...
### 24.134 Command Filename Double-Prefix (Post-delivery — Bug S3-121, NEW IN 2.2)

**Command filename double-prefix (NEW IN 2.2 -- Bug S3-121).** Claude Code's plugin loader registers slash commands as `{plugin_namespace}:{filename_stem}`, prepending the namespace from `plugin.json` and using the filename stem verbatim — no prefix stripping is performed. The SVP plugin declares `name: "svp"` in its manifest, and its `commands/` directory shipped 11 files named `svp_bug.md`, `svp_save.md`, `svp_oracle.md`, `svp_help.md`, `svp_hint.md`, `svp_ref.md`, `svp_redo.md`, `svp_quit.md`, `svp_status.md`, `svp_clean.md`, `svp_visual_verify.md`. The loader therefore registered them as `/svp:svp_bug`, `/svp:svp_save`, `/svp:svp_oracle` and so on — **double-prefixed**. The `# /svp:bug` markdown heading inside each file body is documentation only; it has no effect on registration. Every reference in the spec, blueprint, agent prompts, lessons learned, and user-facing docs correctly cites `/svp:bug`, `/svp:save`, etc. — the documented form never matched the registered form.

**Why it wasn't caught.** Three reinforcing factors: (1) users click through Claude Code's autocomplete list rather than typing the full `/svp:svp_bug` form, so the awkward name was accepted rather than questioned; (2) every document in the repository used the correct `/svp:<name>` form, which matched the `# /svp:<name>` body headings, creating the illusion that filenames and invocations were aligned; (3) S3-87 diagnosed the inconsistency between `svp:orchestration` (correct) and `svp:svp_bug` (broken) in the wrong direction, editing the skill frontmatter to match the broken commands instead of fixing the commands. That edit was also functionally inert because skill registration is driven by the directory path `skills/orchestration/`, not the frontmatter `name` field.

**Cross-project corroboration.** A sibling project (`debrief`) copied SVP's layout as a canonical reference, hit the same bug independently, and documented the diagnosis in its own Bug Catalog (BUG-AUDIT-10). That diagnosis was relayed to this project as an advisory on 2026-04-14 and independently verified against the official Claude Code Plugins Reference documentation by a `claude-code-guide` subagent. The embedded docstring in our own `tests/regressions/test_bug_s3_87_skill_name_prefix.py` already stated the rule correctly — *"Claude Code derives command names from {namespace}:{filename_stem}, so svp_bug.md becomes svp:svp_bug"* — but the assertions in that file codified the wrong conclusion from that rule.

**Fix (Bug S3-121).** Three coordinated changes.

1. **Rename all 11 command files to bare names.** `src/unit_25/stub.py` `COMMAND_NAMES` list and `COMMAND_DEFINITIONS` dict keys rewritten: `svp_help`→`help`, `svp_hint`→`hint`, `svp_ref`→`ref`, `svp_redo`→`redo`, `svp_bug`→`bug`, `svp_oracle`→`oracle`, `svp_save`→`save`, `svp_quit`→`quit`, `svp_status`→`status`, `svp_clean`→`clean`, `svp_visual_verify`→`visual-verify`. After the rename, the loader registers `/svp:bug`, `/svp:save`, `/svp:visual-verify`, etc. — matching every existing document, agent prompt, and lessons-learned citation. Note: `visual-verify` uses a hyphen because the spec §35.18 documents the command as `/svp:visual-verify` consistently; the prior `svp_visual_verify` form drifted on two axes (double-prefix AND underscore vs hyphen) and the rename corrects both. Body-text references to state-machine command names like `svp_bug_entry` and `oracle_start` (the strings passed to `update_state.py --command`) are untouched — those are not filenames.

2. **Invert the S3-87 edit.** `src/unit_26/stub.py` orchestration skill frontmatter `name` reverted from `"svp:svp_orchestration"` back to `"svp:orchestration"` — matching the directory-derived registration. `tests/regressions/test_bug_s3_87_skill_name_prefix.py` renamed to `tests/regressions/test_bug_s3_121_skill_name_correction.py` with inverted assertions: asserts `svp:orchestration` is present and `svp:svp_orchestration` is NOT. The S3-69 hyphen guard (`svp-orchestration` forbidden) is preserved intact.

3. **Defensive cleanup during assembly.** `src/unit_23/stub.py` plugin assembly now unlinks any stale `*.md` files in `commands_dir` before writing the fresh set. Without this, a cached `svp_bug.md` file could survive a reassembly and continue to register as `/svp:svp_bug` alongside the new `/svp:bug`. The cleanup is a negative-space invariant: after assembly, `commands_dir` contains exactly the files `COMMAND_DEFINITIONS` declares and nothing else.

4. **Regression guards.** New file `tests/regressions/test_bug_s3_121_command_double_prefix.py` loads `COMMAND_NAMES` and `COMMAND_DEFINITIONS` from the stub and asserts: (a) no entry starts with `svp_`, (b) every filename matches `^[a-z][a-z_]*$`, (c) for every `(cmd_name, body)` pair the body begins with `# /svp:{cmd_name}` (catches drift between filename and body heading).

**Pattern:** P7 (Specification Omission). The spec did not document the Claude Code registration rule — specifically, that the plugin namespace is prepended to the filename stem with no prefix stripping. Without that rule in the spec, the S3-87 author aligned the skill frontmatter to the broken target instead of recognizing the filenames as the bug. The cross-project advisory pattern (a sibling project hitting the same bug on a canonical reference and relaying the diagnosis back) is a new-to-us detection mode and worth preserving as a deliberate practice. **Detection:** Cross-project advisory from `debrief` BUG-AUDIT-10 + subagent verification against Claude Code Plugins Reference docs + inspection of the embedded docstring in S3-87 regression test.

**User-side cache invalidation.** Claude Code caches resolved plugin contents at `~/.claude/plugins/cache/svp`. After pulling this fix, users must either `claude plugin uninstall svp@svp && claude plugin install svp@svp` or `rm -rf ~/.claude/plugins/cache/svp` to clear the cached old filenames. Otherwise the session will continue to see both the old double-prefixed commands and the new bare-name commands.

### 24.135 Agent Frontmatter Name Drift (Post-delivery — Bug S3-122, NEW IN 2.2)

**Agent frontmatter name drift (NEW IN 2.2 -- Bug S3-122).** Bug S3-122 is the agent-side correlate of Bug S3-121. Same family (a Claude Code loader convention silently transforms one form into another, and the project's documentation/internal references use the wrong side of the transformation), different surface (agents instead of commands), different mechanism (frontmatter name instead of filename stem). Claude Code's plugin loader registers agents by their YAML frontmatter `name` field, NOT by the filename stem (verified against the official Plugins Reference via the `claude-code-guide` subagent on 2026-04-14, and by inspection of the system-reminder which lists all 21 SVP agents with their hyphenated frontmatter names). The plugin namespace `svp` is prepended, so a frontmatter `name: oracle-agent` registers as the subagent_type `svp:oracle-agent`. For agents, the filename is documentation only — the loader ignores it for invocation lookup.

**The drift.** `src/unit_23/stub.py` line 837 (and the duplicate at line 1121 in `regenerate_deployed_artifacts`) contained `name = stem.replace("_", "-")`. The plugin assembly process iterates over agent files in the `agents_dir`, computes the filename stem (e.g., `oracle_agent` from `oracle_agent.md`), converts every underscore to a hyphen (`oracle-agent`), and writes that converted form as the frontmatter `name` field. The result: deployed agent files have `name: oracle-agent` in the YAML frontmatter while the file is named `oracle_agent.md`. Claude Code uses the frontmatter, so the registered subagent_type is `svp:oracle-agent`. Meanwhile every internal SVP reference uses the underscored form: `PHASE_TO_AGENT["bug_triage"] = "bug_triage_agent"` in `routing.py`, `agent_type="bug_triage_agent"` in `invoke_agent` action blocks, `_agent_prepare_cmd("bug_triage_agent")` in the prepare command builder, `AGENT_STATUS_LINES` keys, the `_agent_model_keys` mapping, and the agent files themselves. The orchestrator therefore reads an action block saying `agent_type: "bug_triage_agent"` and must implicitly translate to `subagent_type: "svp:bug-triage-agent"` — a translation rule that exists nowhere in the spec, the blueprint, the orchestration skill instructions, or the code. A literal-minded orchestrator would call `Task(subagent_type="bug_triage_agent")` and crash with `Unknown agent type`.

**Why it hadn't crashed.** The orchestrator (an LLM running the orchestration skill) has been performing the underscore-to-hyphen-and-namespace translation implicitly on every agent invocation, papering over the drift. This is exactly the failure mode S3-121 corrected for commands: the system appears to work because a smart consumer compensates for a silent producer-side transformation, until the day it doesn't (a future runtime that strict-checks `subagent_type`, a future orchestrator that follows the action block more literally, or an introspection tool that reports the registered name without the translation).

**Fix (Bug S3-122).** Delete the `_` → `-` transformation. `src/unit_23/stub.py` lines 837 and 1121 change from `name = stem.replace("_", "-")` to `name = stem`. Frontmatter `name` becomes the bare filename stem (e.g., `oracle_agent`), the registered subagent_type becomes `svp:oracle_agent`, and the entire chain — filename → frontmatter → registered name → action block `agent_type` → `PHASE_TO_AGENT` → `AGENT_STATUS_LINES` — uses one form throughout. This is the symmetric correction of S3-121: there, we standardized commands on the bare-name form the documentation already used; here, we standardize agents on the underscored form the routing tables already use. No other file changes are required: `routing.py`, `prepare_task.py`, the orchestration skill, the spec/blueprint/lessons references — all already use the underscored form.

**Regression guards.** New file `tests/regressions/test_bug_s3_122_agent_name_drift.py` parses the deployed agent frontmatter and asserts: (a) every frontmatter `name` value equals the filename stem verbatim (no transformation), (b) no frontmatter `name` value contains `-` (negative sentinel against the re-introduction of the `_`→`-` transformation), (c) every `PHASE_TO_AGENT` value corresponds to an existing agent filename stem (locks the routing-side reference to the deployed identifier set), (d) every `agent_type` value emitted by `_make_action_block(action_type="invoke_agent", ...)` calls in `routing.py` is in the set of filename stems (drift detector for the action block side). Tests import from the derived `routing` module (not the stub) per the S3-103 stub-import discipline.
...
### 24.137 prepare_task.py Phase/Agent-Type Dispatch Mismatch (Post-delivery — Bug S3-124, NEW IN 2.2)

**Phase/agent_type dispatch mismatch (NEW IN 2.2 -- Bug S3-124).** Two parallel namespaces exist for Group B agents. **Phase names** (used as slash command body arguments and as `PHASE_TO_AGENT` dict keys): `help`, `hint`, `redo`, `bug_triage`, `oracle`, `reference_indexing`. **Agent type names** (used as `prepare_task.py` internal dispatch keys and as `PHASE_TO_AGENT` dict values): `help_agent`, `hint_agent`, `redo_agent`, `bug_triage_agent`, `oracle_agent`, `reference_indexing`. For five of the six agents, the phase name and agent_type name differ by an `_agent` suffix; for `reference_indexing` they are identical. When the human invokes `/svp:help` as a direct slash command, the orchestrator reads the command body in `src/unit_25/stub.py` (line 151) and runs `prepare_task.py --agent help` literally — passing the **phase name**. `prepare_task.py`'s dispatch at `src/unit_13/stub.py:1727` matches only `agent_type == "help_agent"` (the **agent_type** form), so the dispatch falls through to the generic fallback at line 1748 that returns `"# Task Prompt for help\n\nNo specific dispatch for agent type: help"` — an empty placeholder. The help agent is then spawned with no structured task prompt. Affected commands: `/svp:help`, `/svp:hint`, `/svp:redo`. Unaffected: `/svp:ref` (phase == agent_type, lucky), `/svp:bug` and `/svp:oracle` (thin state-transition triggers per Bug S3-119 and Bug S3-79; no longer call `prepare_task.py` from the command body).

**Why it wasn't caught.** Same anti-pattern as S3-122: the LLM orchestrator compensates for the broken producer. When the orchestrator receives the "No specific dispatch" fallback output from `prepare_task.py`, it recognizes the help-agent role from contextual cues in the body text and in the slash command name, and produces a help response from LLM common sense rather than from a structured task prompt. The command *appears* to work because a smart consumer papers over a broken producer. The fallback is only visible if the human reads the raw Bash output — which is how S3-124 surfaced on 2026-04-14 during manual verification of a freshly-built SVP pipeline directory.

**Pre-existing bug.** `git show c75748d:src/unit_25/stub.py` confirms the body strings `prepare_task.py --agent help`, `--agent hint`, `--agent redo`, etc. were present in the initial Pass 2 delivery commit (March 30 2026). Bugs S3-121, S3-122, and S3-123 did not introduce or touch these lines. The bug has existed for the entire lifetime of Pass 2 and was masked by the orchestrator's compensation behavior until the human performed empirical end-to-end verification.

**Half-applied fix as the root cause.** `src/unit_13/stub.py` already has dual-form dispatches for two agents: line 1707 `elif agent_type in ("coverage_review", "coverage_review_agent"):` and line 1737 `elif agent_type in ("bug_triage", "bug_triage_agent"):`. Someone previously encountered the phase/agent_type mismatch for these two agents and applied the correct fix — accept both forms at the dispatch site — but did not extend the fix to `help_agent`, `hint_agent`, `redo_agent`, or `oracle_agent`. A half-applied fix is worse than no fix because it creates the false impression that the pattern is handled while leaving latent breakage for the other cases.

**Fix (Bug S3-124).** Three coordinated changes.

1. **Uniform dual-form dispatch.** `src/unit_13/stub.py` lines 1727, 1729, 1735, 1743 are converted from single-form to dual-form matches, following the existing `bug_triage`/`coverage_review` precedent: `elif agent_type == "help_agent":` becomes `elif agent_type in ("help", "help_agent"):`, and similarly for `hint`, `redo`, `oracle`. Four lines changed. Backward compatible — callers that pass the agent_type form (`help_agent`) still dispatch correctly, AND callers that pass the phase form (`help`) now also dispatch correctly.

2. **No change to slash command bodies.** `src/unit_25/stub.py` is untouched. The body text stays as `--agent help` / `--agent hint` / `--agent redo` (phase names), which reads naturally in user-facing command documentation. The dispatcher fix makes these work without rewriting every documented example. Slash command body text is user-facing content; canonical agent identifiers are an internal concern; keeping them separate is the cleaner design.

3. **Regression guards.** New file `tests/regressions/test_bug_s3_124_prepare_task_phase_dispatch.py` parameterizes over each Group B `(phase, agent_type)` pair and asserts that calling `prepare_task` with EITHER form produces a populated task prompt, NOT the `"No specific dispatch for agent type"` fallback. The test imports from the derived `prepare_task` module per S3-103 stub-import discipline.

**Pattern:** P28 (NEW — Half-Applied Dispatch Fix). When a producer/consumer namespace mismatch is fixed by adding a dual-form dispatch at the consumer, audit ALL similar dispatches in the same function for the same pattern. Stopping at one or two cases creates a partial fix that can silently mask the remaining broken cases for years. Sibling of P26 (Generator-Side Identifier Transformation) and P27 (Default Scope Leak) — all three concern silent drift between what the code says and what the system actually does at runtime. Mitigation checklist: (a) when you find a dual-form dispatch, grep for all single-form dispatches in the same file, (b) treat any remaining single-form dispatch as a latent bug unless its namespace is provably uniform, (c) add a regression test that exercises every dispatch path with both forms. **Detection:** Manual verification during S3-123 empirical testing — the user invoked `/svp:help` in a fresh Claude Code session, read the raw Bash output of the `prepare_task.py --agent help` call, and saw the fallback string. No test suite caught it.

### 24.138 `svp_bug_entry` Orphan Prefix in `dispatch_command_status` (Post-delivery — Bug S3-125, NEW IN 2.2)

**Orphan prefix (NEW IN 2.2 -- Bug S3-125).** `src/unit_14/stub.py` `dispatch_command_status` had 14 `command_type` dispatch branches. Thirteen were bare-named (`stub_generation`, `test_execution`, `quality_gate`, `unit_completion`, `compliance_scan`, `structural_check`, `lessons_learned`, `debug_commit`, `stage3_reentry`, `oracle_start`, `oracle_test_project_selection`, `oracle_gate_7a`, `oracle_gate_7b`). One — `svp_bug_entry` — carried an `svp_` prefix. A 13:1 ratio in favor of bare naming. No other dispatcher shares the `dispatch_command_status` namespace, so the `svp_` prefix on one entry added zero disambiguation value inside a file whose entire purpose is SVP state dispatch. Worse, the asymmetry was **explicitly noted** in the existing test suite at `tests/unit_25/test_unit_25.py:28`: *"bug uses --command svp_bug_entry, oracle uses --command oracle_start"* — the drift had been observed, documented, and treated as accepted state rather than as a bug.

**Historical trace.** `svp_bug_entry` was introduced by Bug S3-119 as a new dispatch handler to allow `/svp:bug` to bootstrap a debug session from pipeline_complete. At the time S3-119 landed, the slash command file was `svp_bug.md` (with prefix), so the new dispatch command name was chosen symmetrically: `svp_bug_entry` to mirror `svp_bug.md`. S3-121 later renamed the file from `svp_bug.md` to `bug.md` but left this internal dispatch key stranded. `svp_bug_entry` has been an S3-119-era artifact living in an S3-121-era codebase ever since.

**Pattern P28 applies.** Half-applied rename: the producer-side (slash command file) was renamed by S3-121 but the consumer-side (state-machine dispatch key and slash command body text that calls it) was left with the old form. Same family as S3-121 (command filenames), S3-122 (agent frontmatter names), and S3-124 (phase/agent_type dispatch): silent drift between naming conventions within a single codebase after an incomplete sweep. The S3-125 fix extends the S3-121 rename to its logical conclusion in the state-machine command namespace.

**Why it wasn't caught during S3-121.** S3-121's plan explicitly scoped out state-machine command names: *"Body-text references to state-machine command names like `svp_bug_entry` and `oracle_start` (the strings passed to `update_state.py --command`) are untouched — those are not filenames."* This was a conservative decision at the time, intended to keep the S3-121 blast radius surgical. But the conservative scoping was wrong: `svp_bug_entry` was the only `svp_`-prefixed state-machine command, and leaving it alone created the 13:1 asymmetry. Scope exclusions in cross-layer renames should be accompanied by an audit of the excluded layer — which would have revealed the outlier and made the exclusion arbitrary.

**Fix (Bug S3-125).** Hard rename `svp_bug_entry` → `bug_entry` everywhere (no dual-form dispatch, no backward compatibility layer). No external callers exist — every reference is in files under SVP's control. The new name is `bug_entry` (not `bug_start`) because the underscore suffix `_entry` preserves the semantic meaning "enter the debug session state" — the spec §12.18.13 transition is `null → "triage"`, an *entry* into the debug state machine, not a generic *start*. `oracle_start` uses `_start` because starting an oracle session is a different semantic operation (creating a nested workspace). The two commands are different operations that should have different verbs; making them symmetric by renaming to `bug_start` would flatten a meaningful semantic distinction.

**Scope.** (a) `src/unit_14/stub.py:3116` dispatch key renamed; line 3112 comment updated to preserve Bug S3-119 attribution and add the S3-125 note. (b) `src/unit_25/stub.py` `_SVP_BUG_DEFINITION` body text renamed (the `--command bug_entry` invocation and the "explanatory error" prose line). (c) `blueprint/blueprint_contracts.md` three live contract bullets renamed (dispatch table entry, PHASE_TO_AGENT description, `/svp:bug` command contract). (d) `tests/unit_25/test_unit_25.py` live assertions renamed, plus the docstring that explicitly called out the asymmetry (now symmetric). (e) `tests/regressions/test_bug_s3_119_svp_bug_bootstrap.py` live `dispatch_command_status` calls renamed; **file NOT renamed** (filenames bind to bug numbers, not command names). (f) New file `tests/regressions/test_bug_s3_125_bug_entry_rename.py` with three guard tests: positive dispatch, negative sentinel against re-introduction of the old name, and body-text check.

**Historical bug catalog entries NOT modified.** The spec's S3-119 catalog entry (Section 24.132) and S3-121 catalog entry (Section 24.134) both reference the original `svp_bug_entry` name in their historical prose. These are historical records describing what was done at the time, not live references. They are preserved unchanged. Only live references (the dispatch key, the command body text, the blueprint contract bullets, and the live test assertions) are renamed.

**Pattern:** P28 (Half-Applied Dispatch Fix), extended to the state-machine command namespace. Fourth member of the bug family: S3-121 (command files), S3-122 (agent frontmatter), S3-124 (phase/agent_type dispatch), S3-125 (state-machine dispatch key). All four share the same failure mode: a partial rename in one layer leaves a dangling reference in another. **Mitigation protocol for future cross-layer renames**: after renaming any surface, grep for the old name across all files under project control, classify every remaining reference as live or historical, and update the live ones. If the scope is deliberately limited, audit the excluded surfaces for consistency. Never leave an asymmetry ambiguous.

### 24.139 CLAUDE.md Tier 1 Lacks Universal Manual Bug-Fixing Protocol and Commit-To-Git Guidance (Post-delivery — Bug S3-126, NEW IN 2.2)

**Missing universal break-glass protocol (NEW IN 2.2 -- Bug S3-126).** The CLAUDE.md split introduced by Bug S3-99 (Section 24.112) placed the Manual Bug-Fixing Protocol (Break-Glass Mode) exclusively in `CLAUDE_MD_SVP_ADDENDUM` (Tier 2), appended only for E/F self-build projects via `enrich_claude_md_for_svp_build()`. For A-D archetypes (`python_project`, `r_project`, `claude_code_plugin`, `mixed`), no break-glass protocol was ever installed in the delivered project's `CLAUDE.md`. When SVP routing was too broken to function in an A-D project, the main session had no written fallback procedure — it had to improvise. Additionally, neither tier prescribed a final **commit to git** step: fixes could be applied and verified but the protocol never closed the loop by recording the change in version control.

**Scope mismatch between tiers.** The original Tier 2 text was SVP-self-build-specific (referencing `src/unit_*/stub.py` as the source of truth, `sync_workspace.sh`, test-from-both-workspace-and-repo, and deployed plugin artifacts under `svp/commands/`, `svp/skills/`, `svp/agents/`, `svp/hooks/`). These concepts are meaningless for A-D projects, which have a single source tree and a simple delivered repo. Simply copying Tier 2 into Tier 1 was therefore not viable — the universal protocol had to be generic, with E/F-specific concerns layered on top as an override.

**Workspace/repo awareness.** All SVP projects (A-F) end up with a delivered repo at `{project_root.parent}/{project_name}-repo/` — a sibling directory of the workspace. The workspace itself is NOT a git repository; git lives only in the sibling delivered repo. Its absolute path is persisted in `.svp/pipeline_state.json::delivered_repo_path` after Stage 5 completes (Bug S3-112). The universal break-glass protocol must tell the orchestrator to read `delivered_repo_path` from state, navigate to the sibling directory, and commit there — never in the workspace.

**Commit message format from profile.** `project_profile.json::vcs.commit_style` (Section 6.4) is `"conventional"`, `"freeform"`, or `"custom"`. For `"custom"`, `vcs.commit_template` holds the template string. The universal protocol instructs the orchestrator to read the profile at commit time and format the message accordingly. When the profile is absent (break-glass may fire before Stage 1 in pathological cases) or `"conventional"` is selected, the default message format is `fix: <bug-id> <short-desc>`.

**Fix (Bug S3-126).** Three coordinated changes.

1. **Promote the Manual Bug-Fixing Protocol into Tier 1** (`CLAUDE_MD_TEMPLATE` in `src/unit_29/stub.py`) as a universal section applicable to every archetype. The universal cycle is: (a) DIAGNOSE — identify root cause; (b) PLAN — spec → blueprint → code; (c) EXECUTE — apply code changes; (d) EVALUATE — run tests; (e) LESSONS LEARNED — record in `references/lessons_learned.md` if the file exists, otherwise skip; (f) REGRESSION TESTS — author tests covering the bug; (g) VERIFY — tests pass with 0 skipped; (h) **COMMIT TO GIT** — read `delivered_repo_path` from `.svp/pipeline_state.json`, `cd` into the sibling directory, read `project_profile.json::vcs.commit_style` and `vcs.commit_template`, format the message per style (default `fix: <bug-id> <short-desc>`), then `git add` and `git commit`.

2. **Rewrite Tier 2** (`CLAUDE_MD_SVP_ADDENDUM`) as an **override addendum** (still E/F only, still appended post-Stage-0 by `enrich_claude_md_for_svp_build()`). Instead of restating the entire cycle, it specifies overrides to Tier 1's universal steps: EXECUTE is overridden to require fixes in `src/unit_*/stub.py` (stubs-as-source-of-truth); new DEPLOYED ARTIFACTS step covers `svp/commands/`, `svp/skills/`, `svp/agents/`, `svp/hooks/`; new SYNC step runs `sync_workspace.sh` before commit; VERIFY is strengthened to require tests passing from BOTH workspace AND repo; the COMMIT TO GIT step from Tier 1 remains the final step (the sync step propagates workspace changes into the delivered repo before the commit executes there).

3. **Update the idempotency marker** in `enrich_claude_md_for_svp_build()`. The previous marker text `"Manual Bug-Fixing Protocol"` is now present in Tier 1 by default, so the old check would always short-circuit and the Tier 2 addendum would never append. The new marker is a string unique to Tier 2 (e.g., `"SVP Self-Build Override"`), which appears only in the Tier 2 override heading.

**Scope.** (a) `src/unit_29/stub.py` — `CLAUDE_MD_TEMPLATE` rewritten to include the universal Manual Bug-Fixing Protocol section with commit-to-git final step; `CLAUDE_MD_SVP_ADDENDUM` rewritten as an override addendum; `enrich_claude_md_for_svp_build()` idempotency marker updated. (b) `blueprint/blueprint_contracts.md` Unit 29 Tier 3 contracts amended to specify the new required contents of both constants and the new idempotency key. (c) `tests/regressions/test_bug_s3_126_universal_break_glass.py` — new regression file with guards on Tier 1 content, Tier 2 override content, idempotency, and absence of commit-step duplication. (d) `references/svp_2_1_lessons_learned.md` — new entry describing the Tier 1/Tier 2 refactor and commit-to-git addition. (e) Unit 23 (`assemble_svp_workspace_artifacts`) is NOT modified — it imports both constants from Unit 29, so changes propagate automatically into the E/F self-build delivered repo.

**Relationship to Section 43.9.** Section 43.9 describes a separate "orchestrator break-glass protocol" that handles in-pipeline exhaustion failures during E/F self-builds (the orchestrator escalates to the human when routing loops or retries exhaust). The CLAUDE.md "Manual Bug-Fixing Protocol (Break-Glass Mode)" is a different concept: it governs what the orchestrator does when SVP routing is *itself* too broken to function and the human has asked for direct manual intervention. Both use the "break-glass" idiom but they are not the same mechanism and neither subsumes the other. This bug entry and the S3-126 fix are scoped to the CLAUDE.md protocol only.

**Pattern:** P7 (Specification Omission). Bug S3-99 documented the Tier 1/Tier 2 split but framed the protocol as an SVP-self-build concern, not as a universal project concern. The omission surfaced only when the user observed that A-D projects also need a fallback procedure and a closing commit. **Detection:** Human review during Pass 2 break-glass mode.

### 24.140 `ensure_project_settings()` Writes a Cache-Parent Path That Has No `marketplace.json` (Post-delivery — Bug S3-127, NEW IN 2.2)

**Missing marketplace.json at the registered path (NEW IN 2.2 -- Bug S3-127).** After the S3-123 migration to project-scope plugin activation, starting a new Claude Code session from an SVP pipeline directory crashed at plugin load time with `Marketplace file not found at /Users/<user>/.claude/plugins/cache/svp/svp/.claude-plugin/marketplace.json`. The path being dereferenced does not exist and cannot exist: `~/.claude/plugins/cache/<marketplace>/<plugin>/` is Claude Code's internal plugin cache layout, **not** a marketplace root. The only `.claude-plugin/marketplace.json` on the machine was in the original source repo at `<svp-source-repo>/.claude-plugin/marketplace.json`. SVP was therefore inaccessible from project-scoped launches despite the S3-123 fix having "landed" and its regression tests passing.

**Root cause chain.** `svp_launcher.main()` calls `_find_plugin_root()` which searches standard locations in order. The second location, `~/.claude/plugins/cache/svp/svp`, contained a leftover snapshot from a previous user-scope install (a `2.2.0/` version subdir with a valid `plugin.json`). `_find_plugin_root()` returned `~/.claude/plugins/cache/svp/svp/2.2.0/`. Control then passed to `ensure_project_settings()` which computed `marketplace_root = plugin_root.parent.resolve()` → `~/.claude/plugins/cache/svp/svp/` and wrote that string into `extraKnownMarketplaces.svp.source.path`. The assumption that "the plugin's parent directory contains `.claude-plugin/marketplace.json`" only holds for the **source-repo layout** (`<repo>/svp/` whose parent `<repo>/` holds the marketplace file). It is false for the **cache layout**, where the parent is just an intermediate directory of Claude Code's internal plugin cache.

**Why the S3-123 test suite didn't catch it.** `tests/regressions/test_bug_s3_123_project_settings.py` fixture `plugin_root` (line 46) constructs a synthetic marketplace with `marketplace_root = tmp_path / "svp_marketplace_repo"` and `plugin_dir = marketplace_root / "svp"`. The test never asserts that a real `marketplace.json` file exists inside `marketplace_root / ".claude-plugin/"`. It only checks that `plugin_root.parent.resolve()` equals what was written into settings.json. The invariant being tested was structural ("parent is written correctly") rather than semantic ("the written path is actually a valid marketplace root"). The cache-layout failure mode was therefore invisible to the suite — the fixture was too forgiving.

**Why this didn't manifest during S3-123's own verification.** When S3-123 was developed, the user still had SVP registered user-scope at `~/.claude/settings.json` with `extraKnownMarketplaces.svp.source.path = <svp-source-repo>`. Claude Code was populating the cache from a real marketplace root. After the S3-123 user-side migration (`claude plugin uninstall svp@svp --scope user` and `"extraKnownMarketplaces": {}` written into user-scope settings), the user-scope pointer to the real marketplace was removed. From that point forward, every `svp resume` rewrote the project's `.claude/settings.json` with the stale cache-parent path — a self-healing mechanism actively **corrupting** its target.

**Section 4.4 already documented the invariant.** The S3-123 common-mistakes checklist in §4.4 states: *"Writing the settings file before the marketplace directory actually exists on disk — the path in `extraKnownMarketplaces.svp.source.path` must resolve to a real directory with `.claude-plugin/marketplace.json` inside."* The checklist was correct. `ensure_project_settings()` did not enforce it. Documentation without enforcement is a permission slip.

**Fix (Bug S3-127).** Four coordinated changes.

1. **New helper `_find_marketplace_root(plugin_root: Path) -> Path | None`** in Unit 29. Resolution order: (a) `SVP_MARKETPLACE_ROOT` env var if set and valid, (b) `plugin_root.parent` if it contains `.claude-plugin/marketplace.json`, (c) walk up from `Path(__file__).resolve()` (the launcher's own location on disk) looking for the nearest ancestor containing `.claude-plugin/marketplace.json` whose `plugins` array lists an entry with `name == "svp"`. Returns `None` if no valid marketplace root is reachable. The `__file__` walk-up is the reliable fallback: the launcher is itself part of the plugin, so its own location on disk transitively identifies a valid marketplace root — provided the launcher was imported from the source tree rather than a site-packages reinstall.

2. **`_find_plugin_root()` becomes a two-pass search.** First pass returns only candidates whose `parent/.claude-plugin/marketplace.json` file exists — this prefers source-repo installs over cache snapshots. Second pass is the existing behavior (any plugin_root with a valid `plugin.json`). The effect is that a user with both a source-repo checkout and a stale cache snapshot gets the source repo. A user with only the cache still falls back to it (and `ensure_project_settings()` then hard-fails with a clear message rather than writing a broken path).

3. **`ensure_project_settings()` hard-fails on missing marketplace.** The helper now calls `_find_marketplace_root(plugin_root)`. If the result is `None`, it raises `FileNotFoundError` with an actionable message: *"Cannot locate a valid SVP marketplace root. Set `SVP_MARKETPLACE_ROOT=<path-to-svp-source-repo>` or run from a directory where the SVP launcher can discover marketplace.json via its own `__file__`."* The self-healing property is widened: the helper now rewrites `extra["svp"]` whenever the stored path does not contain `.claude-plugin/marketplace.json`, not only when it differs from the freshly-computed one. This converts S3-127 from a silent corruption into a loud failure on the next launcher invocation.

4. **Regression guards.** New file `tests/regressions/test_bug_s3_127_marketplace_path.py` asserts: (a) calling `ensure_project_settings` with a plugin_root whose parent has no marketplace.json does NOT silently write that parent path, (b) the exact cache layout (`cache/<m>/<p>/<version>/.claude-plugin/plugin.json`) is rejected with `FileNotFoundError` when no fallback is reachable, (c) `SVP_MARKETPLACE_ROOT` env var is honored over `plugin_root.parent`, (d) a pre-existing stale path (pointing at a directory missing marketplace.json) is self-healed on the next run, (e) `_find_plugin_root()` prefers a source-repo candidate over a cache-layout candidate when both are discoverable.

**Pattern:** P30 (NEW — Invariant Documented But Not Enforced). The S3-123 common-mistakes checklist explicitly warned about writing a marketplace path that doesn't resolve to a directory with `marketplace.json`. The warning was honored by humans during review but not encoded as an assertion in the helper. A year later, a leftover cache snapshot turned the warning into a bug. **Mitigation protocol**: every bullet in a "common mistakes" checklist must be convertible to an assertion or a validation call in the code that the checklist is about; if it cannot be, promote the checklist item to an architectural invariant that the test suite enforces structurally. A checklist is a seed for test cases, not a substitute for them. Sibling of P28 (Half-Applied Dispatch Fix) and P27 (Default Scope Leak): all three are instances of "the code did half the job and the documentation carried the rest."

**Detection:** Human-reported failure on session launch after a `claude plugin uninstall svp@svp --scope user` migration. The user opened Claude Code in the workspace and immediately hit the `Marketplace file not found` error. The plugin simply did not load.

### 24.141 Preflight Detection of Pre-S3-123 User-Scope Plugin Leak (Post-delivery — Bug S3-128, NEW IN 2.2)

**Documented migration, unenforced detection (NEW IN 2.2 -- Bug S3-128).** Bug S3-123 migrated SVP to project-scope plugin activation and documented the user-side migration steps (run `claude plugin uninstall svp@svp --scope user`, then run the launcher in each pipeline directory). The migration is opt-in, and §4.4's migration-story paragraph instructs the user to verify the fix manually (*"Verify with `cd /tmp && claude` — should show no `/svp:*` commands"*). The launcher never detects whether the migration was actually performed: if a user upgrades SVP and never runs the user-scope uninstall, their `~/.claude/settings.json` still contains `enabledPlugins["svp@svp"] = true` and SVP continues leaking into every Claude Code session on the machine indefinitely. The project-scope settings file that S3-123 writes coexists with the user-scope leak without anyone noticing. This is a **silent-residual-state** failure: the fix works, the migration is documented, but there is no runtime signal that the migration is incomplete.

**Why a runtime assertion instead of more documentation.** This is the output of the post-S3-127 runtime enforcement sweep. The sweep audited every documented-but-unenforced invariant from the S3-121 through S3-127 bug family and classified each one as (a) already covered by an existing regression test, (b) already enforced at runtime, or (c) unenforced. The first two categories were all the filename/frontmatter/dispatch invariants — S3-121 is fully covered by `tests/regressions/test_bug_s3_121_command_double_prefix.py`, S3-122 by `tests/regressions/test_bug_s3_122_agent_name_drift.py`, S3-124/S3-125 by their own parameterized tests, and S3-127 by the new `_find_marketplace_root` helper. **The user-scope leak invariant was the only item in category (c).** Adding a structural check for invariants in categories (a) and (b) would duplicate existing enforcement and create drift risk between two assertion sites — which is itself a new bug class. The sweep's real output is therefore narrower than initially scoped: one runtime advisory for the one unenforced invariant, plus the meta-lesson that the existing regression-test corpus is already doing most of the work.

**Fix (Bug S3-128).** Three coordinated changes.

1. **New helper `check_user_scope_svp_leak()` in Unit 29.** Reads `~/.claude/settings.json` defensively (missing file → `None`, corrupt JSON → `None`, non-dict root → `None`, `enabledPlugins` not a dict → `None`). If `enabledPlugins["svp@svp"] == True`, returns an advisory message string instructing the user to run `claude plugin uninstall svp@svp --scope user` and referencing spec §4.4. Otherwise returns `None`. The helper is **read-only** — it does not mutate the user-scope file, does not raise, and does not fail preflight. It is a pure detection function.

2. **Preflight wire-in.** `preflight_check()` gains a new check #2a, called right after the existing "SVP plugin loaded" check. If `check_user_scope_svp_leak()` returns a non-`None` message, preflight prints it to stdout with a `!` advisory marker (following the same pattern as the existing API-credentials advisory at check #3). The check does NOT append to the errors list — a user-scope leak is an advisory, not a failure. Users with legitimate reasons to keep user-scope enablement during a grace period can ignore the warning; preflight still completes successfully and the launcher still runs.

3. **Regression guards.** New file `tests/regressions/test_bug_s3_128_user_scope_leak_detection.py` asserts: (a) `check_user_scope_svp_leak()` returns `None` when `~/.claude/settings.json` does not exist, (b) returns `None` when the file exists but has no `svp@svp` entry, (c) returns `None` when the file exists but `svp@svp` is `False`, (d) returns a non-empty string containing `"svp@svp"` and `"user scope"` when `svp@svp` is `True`, (e) returns `None` on corrupt JSON (no crash), (f) returns `None` when the root value is a list instead of a dict (no crash), (g) `preflight_check()` prints the advisory to stdout when the leak is detected and to verify the line starts with the `!` marker, (h) preflight returns the same errors list regardless of leak state (advisory does not affect pass/fail). Tests use `monkeypatch` to redirect `Path.home()` to a `tmp_path` so they remain hermetic on every developer's machine.

**What the sweep retired.** The audit that produced S3-128 initially scoped four MUST-tier items: I1 (command filename structural check), I2+I8 (agent frontmatter structural check), I6 (this item, user-scope leak detection), and I11 (deployed artifact staleness check). Dogfooding I2+I8 against the real `svp2.2-pass2-repo/svp/agents/` surfaced that the planned invariant was **inverted** — the planned rule (`name == filename_stem.replace("_", "-")`) was the pre-S3-122 bug, and the correct post-S3-122 rule (`name == filename_stem` verbatim) was already enforced by `tests/regressions/test_bug_s3_122_agent_name_drift.py`. I1 was similarly covered by `tests/regressions/test_bug_s3_121_command_double_prefix.py`. I11 (deployed artifact staleness) was deferred to its own standalone bug entry (S3-129, pending) because it requires auditing Unit 23's assembly flow before committing to a design. The sweep collapsed from four items to one. **This narrowing is a feature, not a failure:** the dogfood check caught an inverted-rule bug before it shipped, and the audit confirmed that the existing regression-test corpus already enforces the invariants the sweep was designed to protect. The only gap was the opt-in migration detection, which is what S3-128 fixes.

**Pattern:** **P31 (NEW — Opt-In Migration Without Residual-State Detection)**. A bug-fix migration path is documented for the user to perform manually (e.g., running a CLI command to clean up an obsolete user-scope config). The fix's own code runs correctly on freshly-migrated systems. But systems that upgraded the fix without performing the migration retain stale state indefinitely, and the code has no runtime signal that would tell the user (or a future developer, or a diagnostic script) that the migration is incomplete. **Mitigation protocol**: every opt-in migration step documented in a bug-fix spec entry must be accompanied by a runtime detection function that can report whether the migration is complete for the current user. The detection function does not need to *fix* the residual state — it just needs to *report* it. Silent residual state is the failure mode; loud detection is the fix.

**Detection:** Runtime enforcement sweep post-S3-127, conducted under the break-glass protocol after the user asked for a proactive audit of documented-but-unenforced platform-contract invariants.

### 24.142 Deployed Artifact Round-Trip Regeneration Test (Post-delivery — Bug S3-129, NEW IN 2.2)

**Test-coverage gap in the S3-80 fix (NEW IN 2.2 -- Bug S3-129).** Bug S3-80 (2026-04-04) added `regenerate_deployed_artifacts()` to Unit 23 and the `TestDeployedArtifactFreshness` class to `tests/regressions/test_bug_fix_completeness.py` with four byte-equal freshness tests: commands match `COMMAND_DEFINITIONS`, `SKILL.md` matches `ORCHESTRATION_SKILL`, agent bodies contain their source `_DEFINITION` constant, and `hooks.json` matches `generate_hooks_json()`. This closed the most visible staleness category but left three gaps uncovered:

1. **Hook shell scripts** (`write_authorization.sh`, `non_svp_protection.sh`, `stub_sentinel_check.sh`, `monitoring_reminder.sh`) have no byte-for-byte freshness check against the `generate_*_sh()` functions in Unit 17. A drift in any of the four shell script constants would ship silently because `regenerate_deployed_artifacts` rewrites them on every sync but no test compares the result.
2. **Hook shell script executable bits.** `regenerate_deployed_artifacts` calls `sh_file.chmod(0o755)` but no test verifies that deployed `.sh` files have the executable bit set. A pre-S3-80 era drift that stripped the mode bit would ship silently — and the `non_svp_protection.sh` hook would be non-functional, unblocking every Bash command in SVP sessions (the exact failure mode that S3-80's prevention pattern was designed to prevent).
3. **Agent frontmatter drift on the `description` and `model` fields.** `test_agents_match_source_pass2` uses `source_content in deployed_text` (substring, not byte-equal), which asserts the agent body is present but is blind to frontmatter drift on any field other than `name` (which is covered by `test_bug_s3_122_agent_name_drift.py`). A deployed agent whose `description` was computed from a stale first-non-comment line, or whose `model` was pinned to a value that `project_profile.json`'s `agent_models` no longer references, would pass the existing test while being wrong.

**Why a round-trip test rather than three new per-category tests.** The initial plan was to add three new tests to `TestDeployedArtifactFreshness` — one for hook shell content, one for hook executable bits, one for agent full-file equality. Each of those tests would need to re-derive the expected content using either duplicated assembly logic (a drift risk) or refactored helpers extracted from `regenerate_deployed_artifacts` (a production code refactor, which introduces its own risk). The cleaner approach is a **round-trip regeneration test**: copy the delivered repo to a temporary directory, call `regenerate_deployed_artifacts()` on the copy, then walk both trees and assert byte-for-byte content and `st_mode & 0o777` match. This design uses the production function as its own oracle — the test contains zero duplicated frontmatter/content logic. It covers all three gaps with a single assertion, automatically extends to any new assembly invariant `regenerate_deployed_artifacts` ever gains, and requires no production code changes.

**Dogfood pre-check confirmed zero pre-existing drift.** Per the conservative bug-fix protocol, the round-trip test was run against the real `svp2.2-pass2-repo` before being added to the committed test suite. The pre-check compared 87 deployed files (11 commands, 21 agents, 1 skill, 5 hooks including `hooks.json` + 4 shell scripts, plus tests/regressions files under `svp/tests`) across content and mode and found zero drift. The repo was in the exact state `regenerate_deployed_artifacts` would produce. Safe to lock.

**Fix (Bug S3-129).** Single change in `tests/regressions/test_bug_fix_completeness.py`:

1. **New test `test_no_drift_via_roundtrip_regeneration`** in `TestDeployedArtifactFreshness`. Copies `PASS2_REPO` to a `tmp_path`-based directory via `shutil.copytree(..., symlinks=False)`, calls `regenerate_deployed_artifacts(target)` on the copy, walks both `svp/` trees and for each file under `commands/`, `agents/`, `hooks/`, `skills/` asserts (a) the file exists in both trees, (b) the byte content matches, (c) the Unix permission bits (`stat().st_mode & 0o777`) match. The mode assertion is skipped on Windows (`sys.platform == "win32"`) because Windows `st_mode` semantics differ. `.DS_Store` and `__pycache__` are filtered from both sides.
...
### 24.147 ledger_manager.py Uses Unix-only fcntl, Blocking Windows (Post-delivery — Bug S3-134, NEW IN 2.2)

**Platform-specific import at module level (NEW IN 2.2 — Bug S3-134).** `src/unit_7/stub.py:6` (and derived `scripts/ledger_manager.py:6`) unconditionally imported `fcntl` to implement advisory file-locking via `fcntl.flock` in `append_entry`. `fcntl` is a Unix-only stdlib module — not present in CPython on Windows. The unconditional top-level import made the module fail at load time on Windows with `ModuleNotFoundError: No module named 'fcntl'`, cascading to every importer of `ledger_manager`. The most consequential importer is `scripts/prepare_task.py:22` (`from ledger_manager import get_ledger_path, read_ledger`), which imports at module top-level — so every PREPARE command in any Stage 0–5 pipeline stage crashes immediately on Windows. `scripts/routing.py` uses 5 lazy imports of ledger_manager inside functions; its module-load survives Windows, but any code path that enters those imports still fails. Reported by an external Windows-host bug report (`BUG_REPORT.md` Bug #1).

**Root cause:** Unit 7's blueprint contracts specified JSONL concurrent-append semantics but did not mandate or constrain the locking primitive. The implementation agent chose `fcntl.flock` (natural on Unix) without a platform guard. The spec's toolchain declares Python as primary language but does not enumerate supported host operating systems — Windows parity was implicit and unenforced.

**Fix (Bug S3-134).** Replace the unconditional `import fcntl` in `src/unit_7/stub.py` with a platform-gated wrapper: `if sys.platform == "win32":` imports `msvcrt` and defines `_lock(f)` / `_unlock(f)` using `msvcrt.locking(..., LK_LOCK | LK_UNLCK, 1)`; `else:` imports `fcntl` and defines the wrappers using `fcntl.flock`. Update `append_entry` to call `_lock(f)` / `_unlock(f)` instead of `fcntl.flock(...)`. Locking semantics delta: `fcntl.flock` is advisory whole-file; `msvcrt.locking` is mandatory byte-range starting from current file position. For append-only single-line JSONL writes, both are sufficient and equivalent in practice. No new runtime dependencies added.

**Pattern:** P33 (NEW — Cross-Platform Stdlib Audit). Every stdlib import must be checked for platform availability. Unix-only stdlib modules that have bitten SVP or adjacent Python projects historically: `fcntl`, `termios`, `pwd`, `grp`, `resource`, `select.kqueue`/`select.epoll`. Windows-only: `msvcrt`, `winreg`, `_winapi`. When an operation requires platform-specific stdlib, isolate it in a thin wrapper gated by `if sys.platform == "win32":` with both branches defined at import time (not lazy, not fallback-on-exception — both paths must be parseable and loadable from every platform).

**Detection:** `tests/test_ledger_cross_platform.py` — (a) a functional test that `append_entry` still works on the current platform (exercises the real lock wrapper end-to-end), (b) an AST-structural test that verifies the stub's platform-gated `_lock` and `_unlock` are defined in both branches and that `fcntl` is never imported at module top level. Full Windows validation requires a Windows CI runner (not available in this batch — validation relies on BUG_REPORT's original reporter re-testing on Windows 11).

### 24.148 toolchain.json Never Materialized at Project Root (Post-delivery — Bug S3-135, NEW IN 2.2)

**Stage-boundary artifact gap (NEW IN 2.2 — Bug S3-135).** `scripts/toolchain_reader.py::load_toolchain(project_root)` — called without a `language=` kwarg — expects a file `<project_root>/toolchain.json` (Layer 1 pipeline toolchain). No code path ever materialized this file. Stage 0 produces `project_profile.json` containing `language.primary` and implicit toolchain selection (via `LANGUAGE_REGISTRY[lang]["toolchain_file"]` pointing to `scripts/toolchain_defaults/<template>.json`, i.e. Layer 2). The Layer 2 → Layer 1 conversion was never wired. First consumer is `scripts/infrastructure_setup.py::main()` at Stage 3 entry, which raises `FileNotFoundError: toolchain.json`, blocking the entire Stage 3 bootstrap. Reported by an external Windows-host bug report (`BUG_REPORT.md` Bug #4b).

**Root cause:** The two-layer toolchain design (pipeline at project root vs. language default in `scripts/toolchain_defaults/`) is encoded in `toolchain_reader.load_toolchain`'s signature (`language=None` → Layer 1, `language="python"` → Layer 2) but no unit is designated as the Layer 2 → Layer 1 materialization owner. Spec Section 8 describes the toolchain abstraction but does not specify where the copy happens. `svp_launcher.py:812-818` copies a `plugin_root/toolchain` *directory* to `project_root/toolchain` — this is unrelated dead code from an earlier design and does not produce `toolchain.json`.

**Fix (Bug S3-135).** Add `ensure_pipeline_toolchain(project_root)` helper function to `src/unit_11/stub.py`. It reads `project_profile.json`, maps `profile.language.primary` via `LANGUAGE_REGISTRY` to the matching template filename, and copies `scripts/toolchain_defaults/<template>` → `<project_root>/toolchain.json`. No-op if the file already exists or the profile has no primary language. Invoked from `main()` after `load_profile` and before `load_toolchain`, making `python scripts/infrastructure_setup.py --project-root .` self-healing. A follow-up cycle (Cycle 7, forthcoming) will additionally wire the helper into `_route_pre_stage_3` for the direct-function-call path.

**Pattern:** P34 (NEW — Stage-Boundary Artifact Materialization). Every stage-output that is consumed as a file by a later stage must have an explicit materialization step owned by a single unit. The owner is either (a) the stage's approval handler (e.g., in `dispatch_gate_response`), or (b) the first consumer, self-healing via a helper. When a two-layer design exists (template vs. instance), the promotion path between layers must be encoded in code, not prose.

**Detection:** `tests/unit_11/test_ensure_pipeline_toolchain.py` — (a) no-op when file exists, (b) python profile materializes `python_conda_pytest.json`, (c) R profile materializes `r_renv_testthat.json`, (d) no-op when profile absent or primary language missing.

### 24.149 _route_pre_stage_3 Never Called run_infrastructure_setup (Post-delivery — Bug S3-136, NEW IN 2.2)

**Stage-3 bootstrap never runs (NEW IN 2.2 — Bug S3-136).** `src/unit_14/stub.py::_route_pre_stage_3` (the Stage-2 → Stage-3 transition handler) advanced `stage` from 2 to 3 and saved state, but never invoked `run_infrastructure_setup` — the only code path that derives `total_units` from the blueprint and scaffolds `src/unit_N/` and `tests/unit_N/` directories. The existing `if state.total_units > 0 and state.current_unit is None:` branch was effectively dead code on first entry: `total_units` stays at 0, falls through, and `_route_stage_3`'s `_validate_stage3_completion` immediately emits `gate_3_completion_failure: "No units defined (total_units is 0)"` — despite a fully approved blueprint on disk. The pipeline surfaces the root-cause routing bug as a Stage 3 completion failure, misleading the human into chasing the wrong stage. Reported by an external Windows-host bug report (`BUG_REPORT.md` Bug #4).

**Root cause:** The `_route_pre_stage_3` skeleton was authored assuming infra setup had already run (or would run via some other dispatch). Neither assumption held: there is no `run_command` emitter for infra setup anywhere in routing, and the Stage 0–2 code paths never call it. The `_validate_stage3_completion` check at `total_units == 0` shows the invariant was known; no one wired the producer.

**Fix (Bug S3-136).** In `_route_pre_stage_3`, after `advance_stage(state, "3")` and before the `current_unit`/`sub_stage` assignment, call `ensure_pipeline_toolchain(project_root)` (Cycle 6's helper, materializes toolchain.json if absent) then invoke `run_infrastructure_setup(project_root, profile, toolchain, language_registry, blueprint_dir)` directly as a function call (not via subprocess), then `state = load_state(project_root)` to reflect the `total_units` write that infra_setup made out-of-band. Only execute this block when `state.total_units == 0` (idempotent on repeat entries). Two `save_state` calls are deliberate: first publishes `stage=3` before infra_setup reads pipeline_state.json; second publishes the `current_unit=1` / `sub_stage=stub_generation` assignment.

**Pattern:** P34 (Stage-Boundary Artifact Materialization — see §24.148). Second consumer now closed. Cycle 6 (S3-135) handled `main()` in Unit 11; Cycle 7 (S3-136) handles the routing-called direct path. Both invoke the same `ensure_pipeline_toolchain` helper + `run_infrastructure_setup`.

**Detection:** `tests/unit_14/test_route_pre_stage_3_wiring.py` — (a) when `state.total_units == 0`, monkeypatch verifies `run_infrastructure_setup` is called with the expected kwargs; (b) when `state.total_units > 0`, the infra-setup helpers are NOT called (idempotency).

### 24.150 run_infrastructure_setup Step 1 Never Executed (Post-delivery — Bug S3-137, NEW IN 2.2)

**Conda env never created (NEW IN 2.2 — Bug S3-137).** `src/unit_11/stub.py::run_infrastructure_setup` Step 1 "Environment creation" invoked `_build_env_create_command` to assemble the conda `create` and `install` shell strings into `env_info["commands"]` and `env_info["bridge_packages"]`, then did nothing with them. A comment above the call declared "The actual subprocess execution is deferred to the orchestration layer or skipped when the environment already exists" — but routing, svp_launcher, and every other code path also skipped the execution. Net effect: no conda environment was ever created. Every Stage 3 quality gate subsequently failed with `EnvironmentLocationNotFound: Not a conda environment: ~/.conda/envs/svp-<project>` — fresh SVP projects could not complete a single Stage 3 unit end-to-end without manual `conda create`. Reported by an external Windows-host bug report (`BUG_REPORT.md` Bug #4e).

**Root cause:** Step 1 was authored as a "build the command" phase with execution deferred to a caller that was never written. The pattern is a variant of Pattern E ("Build but don't execute") from the structural-audit plan (`agile-seeking-reef.md`). The deferral annotation is a design smell: when a function comment says "the caller will run this", the caller must be named and exist. In this case, no caller existed and the reference was aspirational.

**Fix (Bug S3-137).** Add a new `_env_exists(env_name, env_manager)` helper that parses `conda env list` and returns whether the env exists (defensively returns False on `FileNotFoundError` so creation is attempted and its real error surfaces). Insert a new Step 4b execution block in `run_infrastructure_setup` after Step 4's import validation and before Step 5's directory scaffolding. The block runs `env_info["commands"]` via `subprocess.run(..., check=True)`, then installs a deduplicated list of `all_packages` (quality tools) + `env_info["bridge_packages"]` (mixed-archetype bridges) + `third_party_modules` (runtime deps extracted from blueprint imports) into the env via `_build_install_command`. The block is idempotent: `_env_exists` gates the entire block, so re-runs skip when the env already exists. Any CalledProcessError propagates as a failure, matching the function's existing "any step failure raises" contract.

Why Step 4b rather than Step 1: at Step 1 only `env_info["commands"]` is known; `all_packages` is built in Step 2 and `third_party_modules` is built in Step 3. Putting execution between Step 4 and Step 5 ensures (a) all package lists are finalized, (b) scaffolding is not left behind if env creation fails.

**Pattern:** P34 (Stage-Boundary Artifact Materialization, existing). Complements Cycles 6 (S3-135) and 7 (S3-136) — with S3-137 landed, the Stage 2 → Stage 3 bootstrap chain now materializes toolchain.json, derives total_units, creates the conda env, and installs packages as a single unit of work.

**Detection:** `tests/unit_11/test_env_creation_executed.py` — (a) env creation + package install subprocess calls fire when `_env_exists` returns False, (b) both are skipped when `conda env list` reports the env present, (c) the install command includes third-party runtime modules extracted from blueprint imports. `tests/unit_11/conftest.py` adds an autouse fixture monkeypatching `subprocess.run` in the infrastructure_setup module to keep all Unit 11 tests hermetic (no actual conda invocation, no pollution of the host environment).

### 24.151 run_quality_gate_main Exits 0 on All Statuses (Post-delivery — Bug S3-138, NEW IN 2.2)

**Quality gate exit-code contract violated (NEW IN 2.2 — Bug S3-138).** `src/unit_15/stub.py::run_quality_gate_main` (the `scripts/quality_gate.py` CLI entry point) called `run_quality_gate(...)` and printed the returned `QualityResult.status` to stdout, then returned without `sys.exit(nonzero)` regardless of whether the gate passed or failed. Per spec §3.7 (lines 3741-3742), run_command status is reported to the orchestrator via **exit code**: exit 0 → `COMMAND_SUCCEEDED`, nonzero → `COMMAND_FAILED: [code]`. Because quality_gate always exited 0, every `QUALITY_RESIDUAL` and `QUALITY_ERROR` run was translated to `COMMAND_SUCCEEDED` by the orchestrator — routing advanced from `quality_gate_a` → `red_run` believing the gate had passed, and formatter/linter/type-checker violations in the generated tests remained invisible until a later stage caught them (if ever). Reported in an external Windows-host bug report (`BUG_REPORT.md` Bug #4f's reasoning, NEW-C2 line in Cycle 9 of the audit plan).

**Root cause:** The CLI entry point was authored with stdout as the "status channel" for the orchestrator to copy into `.svp/last_status.txt`. This conflicts with spec §3.7, which explicitly defines run_command status as exit-code-based. The producer-consumer contract was designed in two places without reconciliation — `run_quality_gate_main` thought stdout carried the signal; `dispatch_command_status` expected `COMMAND_*` strings derived from exit code. Both sides needed adjustment; this cycle fixes the producer side.

**Fix (Bug S3-138).** Append at the end of `run_quality_gate_main`, after the existing print block:
```python
if result.status in ("QUALITY_RESIDUAL", "QUALITY_ERROR"):
    sys.exit(1)
```
`QUALITY_CLEAN` and `QUALITY_AUTO_FIXED` remain exit-0 (gate passed — the auto-fixed run already observed a clean post-fix state per §24.63 / S3-36 design). The printed QUALITY_* string continues to be diagnostic output, still useful for human review of why a failing gate failed; it no longer serves as the status channel.

**Related:** A forthcoming cycle (Cycle 10) will align the consumer side — `dispatch_command_status`'s `quality_gate` branch — to close the full contract. With both landed, the orchestrator's generic `COMMAND_SUCCEEDED`/`COMMAND_FAILED` translation suffices; no QUALITY_*-specific consumer logic is needed.

**Pattern:** Producer-consumer contract consistency. When a script runs as a subprocess under the orchestrator, its status channel is its exit code (spec §3.7). Stdout is diagnostic.

**Detection:** `tests/unit_15/test_s3_138_quality_gate_exit_codes.py` — parametrized test over all 4 QUALITY_* statuses asserting (a) CLEAN and AUTO_FIXED exit 0, (b) RESIDUAL and ERROR raise SystemExit(1), (c) the status string is still printed to stdout in all cases. Two narrow existing tests in `test_unit_15.py` (`test_main_prints_error_status`, `test_main_prints_residual_status`) were also updated to wrap the call in `pytest.raises(SystemExit)` with `code == 1`.

### 24.152 dispatch_command_status quality_gate Strict `==` Misses COMMAND_FAILED: [code] (Post-delivery — Bug S3-139, NEW IN 2.2)

**Consumer vocabulary mismatch with spec §18.3 (NEW IN 2.2 — Bug S3-139).** All four `quality_gate_*` branches of `src/unit_14/stub.py::dispatch_command_status` used strict `==` comparison against `"COMMAND_SUCCEEDED"` and `"COMMAND_FAILED"`. Per spec §18.3 (lines 3741-3742), the run_command failure status is `COMMAND_FAILED: [exit code]` — e.g., `"COMMAND_FAILED: 1"` — not bare `"COMMAND_FAILED"`. The orchestrator's construction of the failure status appends the exit code, so `==` comparison misses the match and routing raises `ValueError: Unknown status for quality_gate at quality_gate_a: COMMAND_FAILED: 1`.

Surfaced after Bug S3-138 fixed the producer side (`run_quality_gate_main` now exits nonzero on RESIDUAL/ERROR). Without this cycle's consumer-side fix, S3-138 is cosmetic: the right exit code is produced, but the resulting `COMMAND_FAILED: [code]` string fails to route and the pipeline crashes instead of retrying.

**Root cause:** Pattern inconsistency in dispatch_command_status. Adjacent branches (`compliance_scan`, `structural_check`, `lessons_learned`, `debug_commit`, `stage3_reentry`) all use substring matching (`"SUCCEEDED" in status_line`). Only the quality_gate branches (four sub-stages) used strict `==`. The substring form naturally handles the spec-defined `COMMAND_FAILED: [code]` suffix; the strict form does not.

**Fix (Bug S3-139).** Change all four `quality_gate_*` branch bodies from `status_line == "COMMAND_SUCCEEDED"` to `"SUCCEEDED" in status_line` and correspondingly `status_line == "COMMAND_FAILED"` to `"FAILED" in status_line`. The ValueError branch is unchanged: non-COMMAND_* strings (e.g., QUALITY_RESIDUAL) continue to raise, which is the correct response per spec — that would indicate a producer-side contract bug (see S3-138 / §24.151).

**Why NOT extend dispatch to accept QUALITY_* (BUG_REPORT #4f's initial approach):** the QUALITY_* vocabulary is internal to `run_quality_gate`; after S3-138 it is never in `last_status.txt`. Accepting it at the consumer would legitimize producer-side non-compliance with spec §18.3's exit-code rule. The reporter self-corrected in #4g; this cycle implements the #4g approach.

**Related:** Pair with Bug S3-138 (producer side). With both landed, quality_gate's run_command contract is fully spec-compliant: exit code is the status channel, substring match absorbs the `[exit code]` suffix.

**Detection:** Four new tests in `tests/unit_14/test_unit_14.py::TestDispatchCommandStatus` asserting that `"COMMAND_FAILED: 1"` routes the same as bare `"COMMAND_FAILED"` across all four quality_gate sub_stages. Existing 8 tests still pass because `"COMMAND_SUCCEEDED"` is a valid substring match for `"SUCCEEDED"`.

### 24.153 unit_flags Never Propagated to resolve_command (Post-delivery — Bug S3-140, NEW IN 2.2)

**Toolchain flag-propagation gap (NEW IN 2.2 — Bug S3-140).** `toolchain_defaults/python_conda_pytest.json` declares a mypy command template `{run_prefix} mypy {flags} {target}` with per-scope flag fields `"unit_flags": "--ignore-missing-imports"` and `"project_flags": ""`. `scripts/toolchain_reader.py::resolve_command` accepts an optional `flags=` parameter that substitutes into the `{flags}` placeholder. `src/unit_15/stub.py::_execute_gate_operations` (the shared core used by all 6 language runners) called `resolve_command(command_template, env_name, run_prefix, str(target_path))` without `flags=`, so `{flags}` resolved to empty and mypy ran at every Stage 3 unit-level gate without `--ignore-missing-imports`. Result: every test file that imports its unit's implementation (`from config import ...` resolving to `src/unit_N/stub.py` via `conftest.py`) produced a false-positive `import-not-found` residual, failing gate B on Unit 1 and every subsequent unit. Reported by an external Windows-host bug report (`BUG_REPORT.md` Bug #4h).

**Root cause:** Contract gap between the toolchain schema (first-class `unit_flags` / `project_flags` fields) and the gate-runner (ignored them). `resolve_command`'s signature has `flags=""` as a hint that propagation was once intended, but the wiring was never completed in Unit 15.

**Fix (Bug S3-140).** In `_execute_gate_operations`, before each `resolve_command` call, parse the operation's name (`quality.<tool>.<op>`), look up `toolchain_config["quality"][tool]["unit_flags"]` (default `""`), and pass as `flags=` to `resolve_command`. Because `_execute_gate_operations` is the shared core for all 6 runners (`_run_python`, `_run_r`, `_run_stan_syntax_check`, `_run_plugin_markdown`, `_run_plugin_bash`, `_run_plugin_json`), the one-function fix covers every language path.

**Scope note.** This cycle fixes unit-level gate execution (Stage 3). `project_flags` (used at Stage 4+ project-scope gates) is out of scope — the gate-runner currently has no signal for scope and treats every invocation as unit-level. A future cycle could add a `scope` parameter to `_execute_gate_operations` if Stage 4+ gates need `project_flags` propagation.

**Pattern:** Schema-consumer wiring completeness. When a configuration schema declares first-class fields (like `unit_flags`/`project_flags`), every consumer must either read them or explicitly document why they are ignored. A silently-ignored field is a latent bug.

**Detection:** `tests/unit_15/test_s3_140_unit_flags_propagation.py` — (a) when `toolchain.quality.type_checker.unit_flags` is set, `resolve_command` receives `flags="--ignore-missing-imports"`; (b) when absent, `flags=""`; (c) non-`quality.*` operation names don't trigger the lookup.
