...
**Step 6 — Update lessons learned document.** The agent appends a new entry to `docs/svp_2_1_lessons_learned.md` in the delivered repository (the authoritative copy) following the established catalog format: bug number, how caught, test file reference, description, root cause pattern classification (P1-P13 or new pattern with definition), and prevention rule. If the bug reveals a new pattern not covered by existing patterns, the agent defines the pattern and adds it to the pattern catalog. The agent also updates the regression test file mapping table, CHANGELOG.md bug count, and README.md bug count in the delivered repository. The agent must NOT write separately to `docs/references/` — a deterministic post-triage sync script (Section 12.17.6) handles copy propagation.
...
**Documentation sync invariant (NEW IN 2.1 — Bug 87 fix).** After triage agent completion, a deterministic sync script (`sync_debug_docs.py`) runs before routing proceeds to Gate 6.2 or the repair fast-path. The script: (1) copies `docs/svp_2_1_lessons_learned.md` to `docs/references/svp_2_1_lessons_learned.md` in the delivered repository (single source of truth: `docs/` wins), (2) copies `docs/svp_2_1_summary.md` to `docs/references/svp_2_1_summary.md`, (3) syncs both documents back to workspace `references/`, (4) stages and commits all dirty documentation files (CHANGELOG, README, lessons_learned, summary). The triage agent writes only to `docs/` — never to `docs/references/` directly. This eliminates the three documentation sync gaps identified in Bug 87.
