...
- Stage 0, `project_profile`: check for `PROFILE_COMPLETE` or artifact existence with fallback (Bug 73) before presenting `gate_0_3_profile_approval`
...
- `sub_stage == "project_profile"`: uses the two-branch routing invariant with artifact fallback (Bug 73). `route()` reads `last_status.txt` and checks whether `project_profile.json` exists. If `last_status.txt` contains `PROFILE_COMPLETE`, OR if `project_profile.json` exists and `last_status.txt` is not `PROFILE REJECTED` or empty, emits a `human_gate` action for `gate_0_3_profile_approval`. Otherwise, emits an `invoke_agent` action for the setup agent. The artifact fallback handles the case where the agent created the profile during the context phase and Gate 0.2 subsequently overwrote `last_status.txt`. Gate 0.3 dispatch: `PROFILE APPROVED` calls `advance_stage` to transition to Stage 1. `PROFILE REJECTED` re-invokes the setup agent (clears `last_status.txt`, keeps `sub_stage`).
