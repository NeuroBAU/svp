...
- **Follow-up bugs filed (not fixed in S3-109):** (1) Delete or properly derive `scripts/adapt_regression_tests.py` (two options: extend `STUB_TO_SCRIPT` to multi-output, or delete and redirect Unit 11 to `generate_assembly_map.py`). (2) Audit `.svp/assembly_map.json` for stale per-unit source paths (multiple `src/unit_23/*.md` entries point at files that no longer exist).
