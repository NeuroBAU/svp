...
- S3-185 (cycle F1, cap-stone) ships `references/extending-languages.md` as the canonical extension-contract doc that frames the two-contract architecture (manifest schema = BEHAVIOR; LANGUAGE_REGISTRY = DISPATCH) and includes a synthetic Rust archetype as a worked example at `scripts/toolchain_defaults/rust_cargo_test.json` and `scripts/primers/rust/`. The synthetic archetype is NOT registered in LANGUAGE_REGISTRY; the schema-conformance test pins the dispatch boundary. See `references/extending-languages.md` for the seven-step playbook for adding a real new archetype.
