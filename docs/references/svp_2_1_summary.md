...
- **Regression test target invariant (§3.6, Bug 74 fix):** All regression tests must import from real implementation modules, never from stubs. Gate C's `check_stub_imports_in_tests` validates compliance. Pre-green-run import switch replaces stub imports with real script imports.
- **Structural Completeness Checking — Four-Layer Defense (§3.21-3.22, Bugs 71-72 fix):** (1) Blueprint checker validates declaration-vs-usage consistency. (2) Integration test author generates registry-handler alignment tests. (3) `structural_check.py` performs five automated checks: dict registry gaps, enum value gaps, unused exports, string dispatch gaps, and stub imports in test files (Bug 74). (4) Triage agent runs structural pre-check with Registry Diagnosis Recipe.
...
- **Pattern P10 (Error-Path Contract Omission):** Spec and blueprint define only the happy-path dispatch transitions. Error-path and edge-case transitions (unexpected test outcomes, fix ladder engagement, gate presentations) are left unspecified. Implementation omits them because no contract requires them. Introduced in Bug 65, expanded in Bugs 66-69, 73 (every stage independently had the same disease). Spec now requires gate reachability and dispatch exhaustiveness invariant (Section 3.6).
- **Pattern P11 (Structural Completeness Gap):** A pipeline component declares registry entries but lacks a structural test verifying the declaration is consumed by dispatch or routing logic. Introduced in Bugs 71-72.
