...
**Bug count:** 87 bugs cataloged across SVP 1.0 through SVP 2.1 (see `svp_2_1_lessons_learned.md`).
...
- **Review Enforcement — Baked Checklists (§3.20, Bug 57 fix, EXPANDED Bug 74 fix):** Mandatory review checklists are baked directly into the agent definitions for the stakeholder reviewer, blueprint checker, and blueprint reviewer. Each checklist requires explicit verification of downstream dependency analysis, Tier 3 contract coverage, per-gate-option dispatch contracts, call-site traceability, re-entry path invalidation, and regression test target verification (Bug 74: no `from src.unit_` imports in test files). Two-tier defense model: LLM-driven review catches root causes at authoring time (Bug 57); Gate C deterministic check catches symptoms at assembly time (Bugs 56, 74).
- **Regression test target invariant (§3.6, Bug 74 fix):** All regression tests must import from real implementation modules (`routing`, `pipeline_state`, etc.), never from stubs (`src.unit_N.stub`). Stubs may diverge from real scripts, causing false-pass scenarios. Gate C's `check_stub_imports_in_tests` validates compliance. The test agent writes tests against stubs for the red run; a deterministic import-switch step before the green run replaces stub imports with real script imports.
- **Structural Completeness Checking — Four-Layer Defense (§3.21-3.22, Bugs 71-72 fix):** (1) Blueprint checker validates declaration-vs-usage consistency at authoring time. (2) Integration test author generates registry-handler alignment tests. (3) `structural_check.py` performs five automated checks: dict registry gaps, enum value gaps, unused exports, string dispatch gaps, and stub imports in test files (Bug 74). (4) Triage agent runs structural pre-check with Registry Diagnosis Recipe.
...
- Checker receives pattern catalog from `svp_2_1_lessons_learned.md`. The preparation script extracts Part 2 of the lessons learned document verbatim and includes it in the checker's task prompt. Produces advisory risk section identifying structural features matching known failure patterns (P1-P12). Does not block approval.
...
- **Pattern Catalog:** Part 2 of `svp_2_1_lessons_learned.md`. Named failure patterns (P1–P12) with instance lists and prevention rules. Blueprint checker receives the catalog for advisory risk analysis.
...
- **Pattern P11 (Structural Completeness Gap):** A pipeline component declares registry entries (gate IDs, status lines, sub-stages) but lacks a structural test verifying the declaration is consumed by the corresponding dispatch or routing logic. Introduced in Bugs 71-72.
- **Pattern P12 (Test Target Mismatch):** Regression tests import from stubs instead of real implementation modules. Tests pass against a simplified stub while the real script has a different (broken) implementation. Introduced in Bug 74. Gate C's `check_stub_imports_in_tests` prevents recurrence.
- **Two-Tier Defense Model (Bugs 56–57, EXPANDED Bug 74):** (1) LLM-driven review catches root causes at authoring time via mandatory checklists baked into reviewer agent definitions (now including regression test target verification). (2) Gate C deterministic check catches symptoms at assembly time via unused function detection and stub import detection. Together they provide defense in depth against the Bugs 52–55 pattern (functions implemented but never wired into dispatch) and the Bug 74 pattern (tests targeting stubs instead of real scripts).
