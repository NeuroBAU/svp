...
**Test-layer review counter (NEW IN 2.2 -- Bug S3-205, cycle K-3).** `PipelineState` gains a top-level field `test_layer_review_count: int` (default `0`) tracking how many times the implementation_agent has emitted `TESTS_FLAWED: [details]` for the `current_unit`. The field is incremented in `dispatch_agent_status` when `implementation_agent + TESTS_FLAWED` is dispatched, surfaced to the human in the `gate_3_3_test_layer_review` prompt for context, and reset to `0` when `current_unit` advances (next unit's count starts fresh). Advisory only -- there is no autonomous limit; the human owns escalation at the gate via the ABANDON UNIT response option (which appends `current_unit` to `deferred_broken_units` per the existing deferred-units pattern). Distinct from `red_run_retries` (which counts test-execution failures, not agent-declared test-layer suspicions).
...
**(NEW IN 2.2 -- Bug S3-209.)** Concurrent-append safety is platform-specific. On POSIX the lock uses `fcntl`; on native Windows it uses `msvcrt.locking`, which locks a region *relative to the current file position*. Because ledgers are opened in append mode, `_lock` and `_unlock` MUST each `seek(0)` before calling `msvcrt.locking(..., 1)` so both operations anchor to byte 0 and reference the same region — otherwise the post-lock `write()` advances the position and `_unlock` targets a never-locked region, raising `PermissionError`. The append-mode write still lands at EOF regardless of the seek. See contract C-7-S209a. (The `fcntl` branch is unchanged.)
...
**(NEW IN 2.2 — Bug S3-214) Implemented-stub guard + non-code unit tolerance.** Two safety additions. (1) Both write sites (`main` and `generate_upstream_stubs`) guard the write with `_is_implemented_stub(path)` — True iff the file exists and no longer carries the `__SVP_STUB__` sentinel (the implementation agent removes it) — and skip an implemented stub unless `--force`/`force=True` is passed, so a Stage-3 re-entry never clobbers a finished implementation. Routing does not pass `--force`. (2) A shared `_parse_tier2_or_doc_only` helper tolerates non-code / delivery units: on a `SyntaxError`/`ValueError` from `parse_signatures`, if the Tier-2 has no `def`/`class` signature (`_SIGNATURE_RE`) it emits a doc-only stub (docstring + `stub_sentinel`) for the delivery unit; a parse error with a signature present is re-raised (a genuine syntax error in a code unit still fails loud). BUG-10's scoped-re-verification (blueprint-diff → affected-unit → `rollback_to_unit` on FIX BLUEPRINT) is deferred to a future IMP-9 enhancement — the exists-guard already removes the destructive consequence. Pattern P96. Cite S3-214.
...
**(NEW IN 2.2 -- Bug S3-211) Provisioning robustness quartet.** Four L4 fixes harden Stage-0 / pre-Stage-3 conda provisioning against defects that surfaced provisioning the `spine` child on native Windows + Miniconda. (1) **PATH-layout-independent conda invocation.** New helpers `_resolve_conda_executable()` and `_conda_cmd()` resolve the conda executable before every `subprocess.run`; `subprocess.run(shell=False)` does no PATHEXT expansion, so a bare `"conda"` fails on Windows where only `condabin\conda.bat` is on PATH. `_resolve_conda_executable` prefers `shutil.which("conda")` (which finds `conda.bat`), then `CONDA_EXE`, then well-known layouts, else `"conda"`; `_conda_cmd` rewrites a leading `conda` token ONLY on `os.name == "nt"` (POSIX is byte-identical; non-conda commands pass through) and is applied at all 5 conda sites (`_env_exists`, Step-4b create + install, `_list_installed_conda_packages`, `install_dep_delta`). (2) **Interpreter-scoped pip.** The `_build_install_command` fallback and the default toolchain JSON / `PYTHON_TOOLCHAIN` copy use `python -m pip` (not bare `pip`) for `install_command`, `install_dev`, and `validate_command`, so the env interpreter's own pip is targeted regardless of PATH ordering. (3) **Idempotent self-heal.** Step 4b now gates on `if env_info["commands"]:` with env-creation nested under `not _env_exists(...)` but package-install running unconditionally for conda — an existing-but-under-provisioned env is topped up (idempotent install) instead of skipping to a doomed verify with no recovery. (4) **Dep-diff parse + first-party filter.** `_parse_blueprint_package_deps` strips balanced backticks before the trailing parenthetical and rejects residual-backtick tokens; `compute_dep_diff` filters the blueprint package set through the new `_first_party_module_roots()` / `_is_first_party_or_stdlib()` so intra-project modules (`core.*`, `plugin.*`, the project's own package) and stdlib never enter `delta_blueprint_only`. Pattern P93. Cite S3-211.
...
The `ALL_GATE_IDS` constant contains all 35 gate IDs. Set equality with `GATE_VOCABULARY` (Unit 14) is verified by a structural test. **(NEW IN 2.2 -- Bug S3-205, cycle K-3)** `ALL_GATE_IDS` includes `gate_3_3_test_layer_review` (the implementation_agent's `TESTS_FLAWED` escalation gate; Section 18.4). The `_GATE_RESPONSE_OPTIONS` mapping (Unit 13) includes the gate's four response options `["TESTS WRONG", "IMPLEMENTATION WRONG", "BLUEPRINT WRONG", "ABANDON UNIT"]`, character-identical to `GATE_VOCABULARY[gate_3_3_test_layer_review]` (Unit 14) per the gate ID consistency invariant. **(NEW IN 2.2 -- Bug S3-207, cycle K-5)** `ALL_GATE_IDS` further includes `gate_3_4_test_generation_blocked` (the test_agent's `TEST_GENERATION_BLOCKED` escalation gate, response options `["RETRY STUB", "FIX BLUEPRINT", "FIX SPEC", "ABANDON UNIT"]`) and `gate_3_5_coverage_ambiguous` (the coverage_review_agent's `COVERAGE_AMBIGUOUS` escalation gate, response options `["RETRY REVIEW", "FIX BLUEPRINT", "FIX SPEC", "OVERRIDE"]`). Both are P90-shaped escalation paths from the K-5 cross-pattern audit (Pattern P90). **(NEW IN 2.2 -- Bug S3-208, cycle K-6)** `ALL_GATE_IDS` further includes `gate_4_4_integration_tests_blocked` (the integration_test_author's `INTEGRATION_TESTS_BLOCKED` escalation gate, response options `["FIX UNIT", "FIX BLUEPRINT", "FIX SPEC", "OVERRIDE"]`). K-6 extends the P90 audit to Stage-4 agents.
...
Unit 14 is the pipeline's central routing engine and test execution dispatcher. It owns the `route()` function (which reads `pipeline_state.json` and returns a structured action block), the `GATE_VOCABULARY` constant (all 35 gate IDs with their response options), the `dispatch_agent_status` function, the `dispatch_gate_response` function, the `dispatch_command_status` function, and the `TEST_OUTPUT_PARSERS` dispatch table (the third per-language dispatch table). The `route()` function's action block `action_type` vocabulary includes: `invoke_agent`, `run_command`, `human_gate`, `session_boundary`, `pipeline_complete`, `pipeline_held`, and `break_glass`. The `pipeline_held` action type is emitted when `PROJECT_CONTEXT_REJECTED` is detected (Section 3.6). The `break_glass` action type is emitted during E/F self-builds when known exhaustion conditions are met (Section 43.9). The routing script is aware of the Hard Stop Protocol (Section 41): during Pass 1, if a builder script bug is detected, the routing script emits appropriate diagnostic context for the orchestrator to follow the Hard Stop procedure.

**Implementation agent test-layer escalation routing (NEW IN 2.2 -- Bug S3-205, cycle K-3).** `GATE_VOCABULARY` includes `gate_3_3_test_layer_review` with response options `["TESTS WRONG", "IMPLEMENTATION WRONG", "BLUEPRINT WRONG", "ABANDON UNIT"]`. The `implementation_agent` allowed-statuses set in `dispatch_agent_status` includes `TESTS_FLAWED` (alongside `IMPLEMENTATION_COMPLETE` and `HINT_BLUEPRINT_CONFLICT`). When the implementation_agent emits `TESTS_FLAWED: [details]`, `dispatch_agent_status` increments `state.test_layer_review_count` and `route()` presents `gate_3_3_test_layer_review` with the agent's structured details surfaced in the gate prompt. `dispatch_gate_response` for `gate_3_3_test_layer_review` has four branches: TESTS WRONG sets `sub_stage="test_generation"`, clears `last_status.txt`, and routing re-invokes the test_agent (regenerate-in-place); IMPLEMENTATION WRONG keeps `sub_stage="implementation"`, clears `last_status.txt`, and routing re-invokes the implementation_agent; BLUEPRINT WRONG sets `stage="2"` + `sub_stage="blueprint_dialog"` and clears `last_status.txt` (mirrors gate_6_2 FIX BLUEPRINT bit-for-bit); ABANDON UNIT appends `current_unit` to `state.deferred_broken_units` and advances to the next unit (uses the existing deferred-units pattern). When `current_unit` advances, `state.test_layer_review_count` resets to `0`. The branch is bounded by the human at the gate (no autonomous limit; pattern P90).

**Integration_test_author escalation routing (NEW IN 2.2 -- Bug S3-208, cycle K-6).** P90 audit extended to Stage-4. `GATE_VOCABULARY` gains `gate_4_4_integration_tests_blocked` with response options `["FIX UNIT", "FIX BLUEPRINT", "FIX SPEC", "OVERRIDE"]`. The `integration_test_author` allowed-statuses gain `INTEGRATION_TESTS_BLOCKED` (alongside `INTEGRATION_TESTS_COMPLETE`). When the agent emits `INTEGRATION_TESTS_BLOCKED: [details]`, routing presents `gate_4_4_integration_tests_blocked` with the agent's structured details surfaced; `dispatch_gate_response` has four branches: FIX UNIT calls `enter_debug_session(state, 0)` to enter break-glass for the affected unit (mirrors `gate_3_completion_failure INVESTIGATE`); FIX BLUEPRINT mirrors gate_6_2 FIX BLUEPRINT (`stage="2"` + `sub_stage="blueprint_dialog"`); FIX SPEC restarts from Stage 1; OVERRIDE acknowledges the failures and advances past integration to Stage 5 (mirrors gate_3_5 OVERRIDE -- the failures are real but the human accepts them and ships). `regression_adaptation` was audited as part of K-6 and is audit-clean: existing `ADAPTATION_NEEDS_REVIEW` → `gate_4_3_adaptation_review` provides the equivalent escalation path. After K-6, the P90 audit is complete across Stage-3 and Stage-4 authoring agents.

**Test agent + coverage_review_agent escalation routing (NEW IN 2.2 -- Bug S3-207, cycle K-5).** P90 cross-pattern audit broadens K-3's escalation pattern to two more Stage-3 agents. `GATE_VOCABULARY` gains `gate_3_4_test_generation_blocked` with response options `["RETRY STUB", "FIX BLUEPRINT", "FIX SPEC", "ABANDON UNIT"]` and `gate_3_5_coverage_ambiguous` with response options `["RETRY REVIEW", "FIX BLUEPRINT", "FIX SPEC", "OVERRIDE"]`. The `test_agent` allowed-statuses gain `TEST_GENERATION_BLOCKED`; the `coverage_review_agent` allowed-statuses gain `COVERAGE_AMBIGUOUS`. When the test_agent emits `TEST_GENERATION_BLOCKED: [details]`, routing presents `gate_3_4_test_generation_blocked`; `dispatch_gate_response` has four branches: RETRY STUB resets `sub_stage="stub_generation"` so the deterministic stub_generator re-runs (useful for transient framework issues); FIX BLUEPRINT mirrors gate_6_2 FIX BLUEPRINT (`stage="2"` + `sub_stage="blueprint_dialog"`); FIX SPEC restarts from Stage 1 (`stage="1"` + targeted spec revision); ABANDON UNIT appends to `deferred_broken_units` (mirrors K-3 ABANDON UNIT). When the coverage_review_agent emits `COVERAGE_AMBIGUOUS: [details]`, routing presents `gate_3_5_coverage_ambiguous`; `dispatch_gate_response` has four branches: RETRY REVIEW keeps `sub_stage="coverage_review"` and clears `last_status.txt` (re-invoke the agent; useful if it missed an existing test); FIX BLUEPRINT and FIX SPEC mirror the corresponding gate_3_4 branches; OVERRIDE advances to `sub_stage="unit_completion"` (less drastic than ABANDON UNIT since the unit's tests already pass at coverage_review -- we accept the unmet coverage concern and complete the unit). Both gates are human-bounded (no autonomous retry); P90 falsifiability principle preserved (agents must not declare `*_COMPLETE` while a known upstream-layer issue is unresolved).
...
**(NEW IN 2.2 — Bug S3-212) Routing pair fixes.** Two L4 fixes. (1) **gate_1_2 REVISE dead-end.** `dispatch_gate_response` for `gate_1_2_spec_post_review` REVISE now `advance_sub_stage(state, "targeted_spec_revision")` (after `_clear_last_status`), mirroring the `gate_2_3_alignment_exhausted` REVISE SPEC branch, so post-review revisions are applied via `stakeholder_dialog` in `targeted_revision` mode. The previous `_copy(state)` left `sub_stage="spec_review"`, which re-invoked `stakeholder_reviewer` on the unchanged spec — a dead-end blocking the OVERRIDE-4 spec-hardening loop. (2) **pytest count-regex overmatch.** `_parse_pytest_output` now reads the pass/fail/error count regexes from the new `_pytest_summary_count_source(output)` helper — the inner text of pytest's LAST `=`-delimited result-summary line — in both the collection-error and normal branches, falling back to the full output when no banner is present. This extends the S3-196 / P80 authoritative-signal anchoring from collection-error detection to the count regexes, so an echoed docstring like `REQ-PSF-01 error.` no longer overmatches `(\d+)\s+error` and misclassifies `TESTS_FAILED` as `TESTS_ERROR`. Patterns P94 (gate-branch-reaches-its-action) + P80 (extended). Cite S3-212.
...
**(NEW IN 2.2 — Bug S3-215) `sync_debug_docs` ships references/.** In addition to the stakeholder spec and blueprint files, `sync_debug_docs` now copies the workspace `references/` tree into the delivered `docs/references/` (recursive `shutil.copytree(..., dirs_exist_ok=True)`), so the delivered repo carries the lessons-learned / auditor-guide provenance surface. This makes true the function's own "specs, blueprint, references" promise (and the `assemble_python_project` caller comment) — previously only spec + blueprint were shipped. Silent no-op when `references/` is absent; the existing spec/blueprint shipment is preserved. Pattern P97. Cite S3-215.
...
**(NEW IN 2.2 — Bug S3-213) Hook commands anchored to `${CLAUDE_PLUGIN_ROOT}`.** All four `HOOKS_JSON_SCHEMA` command strings use the `${CLAUDE_PLUGIN_ROOT}/hooks/X.sh` prefix rather than a bare relative `.claude/scripts/X.sh`. Claude Code resolves hook commands against the session's working directory, so a relative path (and `${CLAUDE_PROJECT_DIR}`, which is the session launch dir) misses when the SVP project is opened as a subdirectory below the session root — producing a recurring "No such file or directory" on every Bash call and silently disabling the `non_svp_protection` PreToolUse guard. `${CLAUDE_PLUGIN_ROOT}` is the plugin's own install directory (the scripts ship at `svp/hooks/*.sh`), expanded at hook-execution time and CWD/nesting-agnostic. The launcher still deploys copies into `.claude/scripts/` for compatibility (now vestigial for hook resolution). Residual: `write_authorization.sh` still reads `.svp/pipeline_state.json` relative to CWD, so a nested session cannot locate state — a deeper Claude Code limitation tracked as a future enhancement. Pattern P95. Cite S3-213.
...
**Implementation agent honest test-layer escalation (NEW IN 2.2 -- Bug S3-205, cycle K-3).** `IMPLEMENTATION_AGENT_DEFINITION` is amended to forbid the agent from declaring `IMPLEMENTATION_COMPLETE` while ANY test fails -- no agent-side classification of test failures as "test design flaw" or otherwise excused. The agent gains a third terminal status `TESTS_FLAWED: [details]` for use when it has tried to make tests pass and concluded the test layer itself is wrong (test fixture bugs, incorrect assertions, statistical-distribution flaws, etc.). The `[details]` payload MUST describe which tests fail, why they appear flawed, and what the implementation actually does. `TESTS_FLAWED` is reserved for genuine test-layer suspicion; it MUST NOT be used to escape difficult implementation bugs. The cross-agent `HINT_BLUEPRINT_CONFLICT: [details]` status remains scoped to "human-provided hint contradicts the blueprint contract" and is NOT a fallback for test-layer issues. The three terminal statuses (`IMPLEMENTATION_COMPLETE`, `TESTS_FLAWED`, `HINT_BLUEPRINT_CONFLICT`) are mutually exclusive: the agent emits exactly one. Routing dispatches `TESTS_FLAWED` to `gate_3_3_test_layer_review` (Unit 13/14), where the human reviews the agent's structured details and chooses TESTS WRONG / IMPLEMENTATION WRONG / BLUEPRINT WRONG / ABANDON UNIT. Pattern P90 (agents must not auto-classify upstream-layer failures; honest escalation paths > false completion).

**Test agent F841 unused-variable prohibition (NEW IN 2.2 -- Bug S3-206, cycle K-4).** `TEST_AGENT_DEFINITION`'s `## Prohibited Patterns` section is extended with a new P4 prohibition: do not leave unused variables in test code (`F841` ruff lint). Without the prohibition, the test_agent recurringly emitted F841-tripping code (4 of 6 WGCNA Python units in the first half of the pipeline); Gate A failed; the fix-ladder routed to the implementation_agent which had to fix the test file's lint issue in addition to producing the unit's implementation. Each occurrence wasted one fix-ladder cycle. P4 explicitly lists the common offending shapes (`result = func(...)` without an `assert`; named return-bindings used as documentation; tuple-unpack with silently ignored elements) and the discipline (every assigned variable must be used, asserted on, or replaced with `_`; bare expression statements for hidden-side-effect calls). Pattern P14 (existing -- Agent Definition Gap; an explicit prohibition in the agent's definition prevents a recurring defective output pattern). No routing change; sibling P14 fixes already targeted the test_agent for `pytest.skip`, stub-vs-contract testing, `src.`-prefix imports, and terminal-status names -- F841 is the next sibling.

**Integration_test_author honest upstream-defect escalation (NEW IN 2.2 -- Bug S3-208, cycle K-6).** Cross-pattern P90 audit extended to Stage-4. `INTEGRATION_TEST_AUTHOR_DEFINITION` is amended to forbid declaring `INTEGRATION_TESTS_COMPLETE` when integration tests fail -- the prompt explicitly forbids the "pre-existing failures" lie pattern (the WGCNA bug report Section 8 antipattern: agent claimed "the 4 failures are pre-existing" when the project was at 1098/1098 immediately before integration). The agent gains a second terminal status `INTEGRATION_TESTS_BLOCKED: [details]` for use when integration tests reveal a real upstream defect (cross-unit contract mismatch in already-shipped Stage-3 units, implementation bug in a unit the integration tests exercise). The `[details]` payload MUST describe (a) which integration tests fail, (b) what the failure reveals, (c) the suspected affected unit number(s). Routing dispatches `INTEGRATION_TESTS_BLOCKED` to `gate_4_4_integration_tests_blocked` (Unit 13/14) where the human chooses FIX UNIT (enter break-glass for the affected unit) / FIX BLUEPRINT / FIX SPEC / OVERRIDE. The two terminal statuses (`INTEGRATION_TESTS_COMPLETE`, `INTEGRATION_TESTS_BLOCKED`) are mutually exclusive: ALL pass = COMPLETE; any fail with upstream root cause = BLOCKED. The mechanical test runner is the authority on completion. Integration_test_author does NOT receive human hints, so `HINT_BLUEPRINT_CONFLICT` is intentionally absent from its allowed-statuses (preserves the existing exclusion). Pattern P90 (existing; this is a P90 audit-and-broaden cycle).

**Test agent + coverage_review_agent honest upstream-broken escalation (NEW IN 2.2 -- Bug S3-207, cycle K-5).** Cross-pattern P90 audit broadens the K-3 implementation_agent escalation pattern to the two other Stage-3 agents. `TEST_AGENT_DEFINITION` is amended to forbid declaring `TEST_GENERATION_COMPLETE` when the test agent has tried to write tests against a broken upstream artifact (uncompilable stub, incomplete blueprint contract, ambiguous spec) -- specifically, the agent MUST NOT fabricate `sys.modules` injections, dummy contract assertions, or other workarounds to make the imports superficially pass. The agent gains a third terminal status `TEST_GENERATION_BLOCKED: [details]` for use when the upstream artifact is broken; the `[details]` payload MUST describe what the agent was unable to do, which upstream artifact appears broken (stub / blueprint / spec), and what the agent observed (error message, missing field, ambiguity). Routing dispatches `TEST_GENERATION_BLOCKED` to `gate_3_4_test_generation_blocked` (Unit 13/14) where the human chooses RETRY STUB / FIX BLUEPRINT / FIX SPEC / ABANDON UNIT. The three terminal statuses (`TEST_GENERATION_COMPLETE`, `TEST_GENERATION_BLOCKED`, `HINT_BLUEPRINT_CONFLICT`) are mutually exclusive. `COVERAGE_REVIEW_AGENT_DEFINITION` is amended to forbid declaring `COVERAGE_COMPLETE: tests added` when the apparent gap is rooted in a contract ambiguity rather than a missing test (the agent MUST NOT synthesize tests from guessed contract semantics). The agent gains a third terminal status `COVERAGE_AMBIGUOUS: [details]`; the `[details]` payload MUST describe the apparent gap, why the gap is upstream-rooted, and what the agent would test if the contract were clarified. Routing dispatches `COVERAGE_AMBIGUOUS` to `gate_3_5_coverage_ambiguous` where the human chooses RETRY REVIEW / FIX BLUEPRINT / FIX SPEC / OVERRIDE. Both prompts cite the K-3 IMPLEMENTATION_AGENT_DEFINITION's "honest escalation paths > false completion" framing as the precedent. Pattern P90 (existing; this is a P90 audit-and-broaden cycle).
...
**(NEW IN 2.2 — Bug S3-215) Python assembly hardening: LICENSE + post-assembly collect gate.** `assemble_python_project` gains two steps before `return repo_dir`. (1) It writes a `LICENSE` at the delivered repo root via `_r_license_content(profile)` — archetype-neutral despite the `_r_` name (dispatches on `profile["license"]["type"]` across Apache-2.0/MIT/GPL-3.0/BSD-3-Clause, default MIT). Previously only the R path shipped a LICENSE, so a delivered Python repo violated the S3-50 completeness set. (2) It calls the new `_post_assembly_collect_check(repo_dir, package_name)` as the last step: a deterministic `python -m pytest --collect-only` smoke gate over the delivered `tests/` that RAISES on a genuine collection error (a non-collecting harness — stub-synthesis conftests, leftover `import stub`, test syntax errors) but SKIPS cleanly when there are no tests, pytest is unavailable, pytest reports collected/no-tests, or the errors are dominated by the delivered package being un-importable (`No module named '<package>'` — the expected "run `pip install -e .` first" state, not an assembly defect). BUG-14 of the spine memo (delivered-CLAUDE.md Gate-6 leak) is deferred to a future enhancement — its Gate-6 protocol is intentional, test-locked design (S3-126/187/199). Pattern P97. Cite S3-215.
...
**(NEW IN 2.2 -- Bug S3-209, cross-platform installability.)** The launcher is the primary Windows install surface, and several of its behaviors were hardened so `svp new` works on native Windows (PowerShell + Anaconda/Miniconda, not WSL2), where the default text codec is cp1252 rather than UTF-8. Three launcher-specific contracts changed. (1) `preflight_check` gains a `declared_languages` parameter that governs the severity of the language-runtime step: at `svp new` (no language declared yet — the profile is chosen later inside the session) a missing runtime is advisory and does not block startup; at `svp resume`/`restore` (language fixed by the profile) a missing runtime for the declared language is a hard error. Python is always hard via the Python-version step. This lets a Python-only user start SVP without every registry language's runtime installed, while still gating a committed language. (2) `_find_plugin_root()` gains a launcher-relative `__file__` fallback (consulted only when neither `SVP_PLUGIN_ROOT` nor any standard install location yields a candidate), so the plugin resolves from a source or profile-function checkout even in a fresh shell where the User-scope `SVP_PLUGIN_ROOT` has not loaded; standard-location candidates are deduped. (3) The launcher module ends with an `if __name__ == "__main__"` guard so the Windows no-pip profile-function install can invoke `python svp/scripts/svp_launcher.py` directly in place of the pip-generated `svp.exe`. See contracts C-29-S209a/b/c and §6.1.2.

**(NEW IN 2.2 -- Bug S3-209, cross-cutting text-I/O encoding.)** Across all units, every SVP-controlled text file read/write passes `encoding="utf-8"` explicitly, and every CLI entry point that prints non-ASCII (the launcher, routing, `run_tests`, `prepare_task`) reconfigures stdout/stderr to UTF-8 at process entry via a best-effort `_ensure_utf8_streams()` helper. These are the file-I/O and CLI-stream siblings of the P84 subprocess UTF-8 hygiene (§18.3): no-ops where the OS default is already UTF-8, correctness fixes on Windows cp1252. See contract C-29-S209d. The repo also ships per-OS install paths — the native-Windows PowerShell installers (`install_windows.ps1` pip-based; `setup_svp_user.ps1` no-pip profile-function) alongside the macOS/Linux path — because a passing macOS suite does not imply Windows correctness; each supported OS needs its own install path and verification. **(macOS framework Python -- Bug S3-210.)** The documented macOS/Linux pip path uses `pip install -e . --user` (not `--prefix ~/.local`): on a macOS **framework** build (MacPorts / python.org) the only scanned user site is `~/Library/Python/X.Y/lib/python/site-packages`, so `--prefix ~/.local` installs the package off `sys.path` and `import svp` fails even though the console script runs. `pyproject.toml` declares a `test` extra so `pip install -e '.[test]'` satisfies the pytest preflight, and `setup_svp_user.sh` (the POSIX sibling of `setup_svp_user.ps1`) provides a no-pip `svp` shell function that runs `svp_launcher.py` directly — immune to both the framework-site issue and repo relocation.
