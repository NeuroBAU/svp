...
- `complete_unit` writes a marker file to `.svp/markers/unit_N_verified` with `VERIFIED: {timestamp}`, updates the `verified_units` list, resets fix ladder and red run retries, and advances `current_unit`. When `current_unit` exceeds `total_units`, advances stage to "4".
...
- `create_conda_environment` creates the environment using `conda create` and installs packages. Uses `conda run -n {env_name}` for all operations (spec Section 4.3).
...
- `run_pytest` executes `conda run -n {env_name} pytest {test_path} -v` and constructs the appropriate command result status line from the output. Never uses bare `python` or `pytest`.
