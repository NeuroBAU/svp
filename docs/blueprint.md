...
- Gate 6.2 (`gate_6_2_debug_classification`) dispatch: `FIX UNIT` calls `set_debug_classification`, enters `"stage3_reentry"` phase for affected unit(s). `FIX BLUEPRINT` initiates targeted blueprint revision, restarts from Stage 2 (full pipeline re-entry). `FIX SPEC` initiates targeted spec revision, restarts from Stage 1.
- **`stage3_reentry` phase routing:** When `debug_session.phase == "stage3_reentry"`, `route()` performs a Stage 3 re-entry for the affected unit(s). The exact state transition is: set `stage: "3"`, `current_unit: N` (where N is the affected unit from `debug_session.affected_units`), `sub_stage: "implementation"` (preserving existing tests -- the bug is in the implementation, not the tests), `fix_ladder_position: None`, `red_run_retries: 0`. The `verified_units` list is not modified -- previously verified units remain verified; the pipeline processes only the affected unit. Quality Gates A and B run normally during re-entry. After the affected unit completes (all tests pass, quality gates pass), `route()` transitions to the repair success path: reassembly and debug completion (Section 12.17.6), which runs all tests (unit, regression, integration), performs full Stage 5 repo reassembly, updates lessons learned, and presents `gate_6_5_debug_commit`.
