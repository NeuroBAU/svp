...
The Python assembler helper now performs **two delivery steps automatically** after creating the repo structure: (1) copies `workspace/tests/` to `repo/tests/` with a fixed exclusion list (caches, backups, cross-unit workspace stubs), and (2) adapts test imports to match the delivered source layout (no-op for `svp_native`; rewrites flat imports to package-prefixed form for `conventional` / `flat`).

You **MUST NOT** re-do these steps manually. The helper is idempotent and deterministic; running them again from your shell would not produce a better result and risks divergence between the workspace contents and the delivered repo. If you observe missing tests in the delivered repo, treat it as a bug in the assembler helper (file via `/svp:bug`), not as a step for you to compensate for.
