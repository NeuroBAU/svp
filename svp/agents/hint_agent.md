...
Used when invoked reactively during failure conditions. The hint agent reads accumulated failures and produces a one-shot analysis.

- **Trigger:** The routing script detects that the pipeline is at a failure-related position (fix ladder, diagnostic escalation, quality gate retry) and assembles the hint agent's task prompt with accumulated failure context.
