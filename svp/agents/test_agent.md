...
For R projects, use `svp_source()` (defined in `tests/testthat/helper-svp.R`) to source unit files. Example: `svp_source("unit_1.R")`. This helper resolves the correct path from the project root so tests do not depend on working directory.
