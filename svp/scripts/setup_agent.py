...
Provides the complete markdown agent definition for the setup agent,
along with structural validation targets for Rules 1-4, Area 0 archetypes,
and dialog areas.
"""
...
REQUIRED_RULES: list = [
    (
        "Rule 1: Plain-language explanations required. For every choice "
        "presented, the setup agent must explain what each alternative means "
        "in non-technical language that a domain expert can understand. No "
        'jargon without explanation. If a term is technical (e.g., "linter," '
        '"type checker," "semantic versioning," "docstring convention"), the '
        "agent must define it in one sentence before presenting it as an option."
    ),
    (
        "Rule 2: Best-option recommendation required. For every choice, the "
        "setup agent must recommend the best option with a brief rationale. "
        'The recommendation must be clearly marked (e.g., "Recommended: ...") '
        "so the user can simply accept it without evaluating alternatives."
    ),
    (
        "Rule 3: Sensible defaults that always produce a correct project. If "
        "the user has no preference and accepts every recommendation, the "
        "result must be a correct, well-configured project with no gaps or "
        "inconsistencies."
    ),
    (
        "Rule 4: Progressive disclosure. Lead with the recommendation and a "
        "one-sentence explanation. Only provide detailed comparisons between "
        "alternatives if the user asks for more information or explicitly "
        "declines the recommendation."
    ),
]

AREA_0_ARCHETYPES: list = [
    "A. A Python software project",
    "B. An R software project",
    "C. A Claude Code plugin",
    "D. A mixed-language project",
    "E. SVP self-build: language extension",
    "F. SVP self-build: architectural change",
]
...
# ---------------------------------------------------------------------------
# Build the rules block so each rule appears as a single contiguous substring
# ---------------------------------------------------------------------------

_RULES_BLOCK = "\n\n".join(f"**{rule}**" for rule in REQUIRED_RULES)

# ---------------------------------------------------------------------------
# Build the archetype block so each label appears as a contiguous substring
# ---------------------------------------------------------------------------
...
_ARCHETYPE_DESCRIPTIONS = {
    ...
    "D. A mixed-language project": "Python and R as peers. One language owns the project structure; "
    "the other is embedded.",
...
# ---------------------------------------------------------------------------
# Setup Agent Definition (complete markdown)
# ---------------------------------------------------------------------------

SETUP_AGENT_DEFINITION: str = (
    """# Setup Agent
...
"""
    + "\n".join(
        f"{label} -- {desc}"
        for label, desc in _ARCHETYPE_DESCRIPTIONS.items()
        if not label.startswith("E") and not label.startswith("F")
    )
    + """
...
"""
    + "\n".join(
        f"{label} -- {desc}"
        for label, desc in _ARCHETYPE_DESCRIPTIONS.items()
        if label.startswith("E") or label.startswith("F")
    )
    + """
