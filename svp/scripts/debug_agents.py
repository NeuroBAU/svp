...
The `assembly_map.json` file provides bidirectional path mapping between the workspace \
layout and the delivered repository layout:

- `workspace_to_repo`: maps workspace paths (e.g., `src/unit_N/module.py`) to \
delivered repo paths (e.g., `svp/scripts/module.py`).
- `repo_to_workspace`: the inverse mapping.

When the human reports a bug using delivered repo paths, use the `repo_to_workspace` \
mapping to locate the corresponding workspace source files for investigation. When \
referencing files in your triage output, include both workspace and repo paths for \
clarity.
