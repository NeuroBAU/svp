...
def copy_toolchain_default(plugin_root: Path, project_root: Path) -> None:
    """Copy toolchain default to toolchain.json at project root."""
    src = plugin_root / "scripts" / "toolchain_defaults" / "python_conda_pytest.json"
    if not src.exists():
        raise RuntimeError(f"Toolchain default file not found at {src}")
    dst = project_root / TOOLCHAIN_FILE
    shutil.copy2(str(src), str(dst))


def copy_ruff_config(plugin_root: Path, project_root: Path) -> None:
    """Copy ruff.toml from plugin toolchain_defaults to project root (NEW IN 2.1)."""
    src = plugin_root / "scripts" / "toolchain_defaults" / "ruff.toml"
    if not src.exists():
        raise RuntimeError(f"Ruff config file not found at {src}")
    dst = project_root / RUFF_CONFIG_FILE
    shutil.copy2(str(src), str(dst))


def copy_regression_tests(plugin_root: Path, project_root: Path) -> None:
    """Copy regression test files from plugin to project workspace. Non-fatal if missing."""
    src_dir = plugin_root / "tests" / "regressions"
    if not src_dir.exists():
        return

    dst_dir = project_root / "tests" / "regressions"
...
def _handle_new_project(args: argparse.Namespace, plugin_dir: Path) -> int:
    ...
    # Copy toolchain default
    print("Copying toolchain configuration...")
    copy_toolchain_default(plugin_dir, project_root)
    _print_status("Toolchain", True, "Copied")

    # Copy ruff config (NEW IN 2.1)
    print("Copying ruff configuration...")
    copy_ruff_config(plugin_dir, project_root)
    _print_status("Ruff config", True, "Copied")

    # Copy regression tests
    print("Copying regression tests...")
    copy_regression_tests(plugin_dir, project_root)
    _print_status("Regression tests", True, "Copied")
    ...
    copy_hooks(plugin_dir, project_root)
