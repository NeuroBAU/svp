...
2. Run `python scripts/update_state.py --command oracle_start --project-root .` \
to enter the oracle session.
3. Run `python scripts/routing.py --project-root .` to receive the next action \
block.
4. Follow the six-step action cycle from there (the routing script handles test \
project selection, agent invocation, and all oracle phases).
...
- This is a Group B command: after the initial state transition, it follows the \
standard routing-driven action cycle.
- The routing script handles test project selection deterministically. Do NOT \
scan directories or build test project lists.
