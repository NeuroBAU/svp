...
**PHASE_TO_AGENT:**
- Maps `--phase` CLI values to agent type strings: `{"help": "help_agent", "hint": "hint_agent", "reference_indexing": "reference_indexing", "redo": "redo_agent", "bug_triage": "bug_triage_agent", "oracle": "oracle_agent", "checklist_generation": "checklist_generation", "regression_adaptation": "regression_adaptation"}`. **(Bug S3-86 fix: "bug_triage" now maps to "bug_triage_agent")**
- Used by `update_state_main` to validate that the `--phase` argument corresponds to a known agent type.
- **Namespace consistency invariant (Bug S3-86):** Every value in PHASE_TO_AGENT must exist as a key in AGENT_STATUS_LINES and must have a corresponding handler in `dispatch_agent_status`. A structural regression test verifies this.
...
- Every `human_gate` action block includes a `post` field: `python scripts/update_state.py --command <gate_id> --project-root .`. The `dispatch_command_status` catch-all matches any `command_type` in `GATE_VOCABULARY` and routes to `dispatch_gate_response(state, command_type, status_line, project_root)`. This ensures gate responses are processed into state transitions without orchestrator improvisation. **(Bug S3-85 fix)**
- Oracle routing: if `oracle_session_active` is True, dispatches on `oracle_phase` instead of normal stage/sub_stage. Per-phase routing:
  ...
  - `oracle_phase == "gate_a"`: emit `human_gate` with `gate_id: "gate_7_a_trajectory_review"`, `post: "python scripts/update_state.py --command oracle_gate_7a --project-root ."`. Present trajectory plan for human review. **(Bug S3-82 fix.)**
  - `oracle_phase == "green_run"`: if `last_status == "ORACLE_FIX_APPLIED"`, emit `human_gate` with `gate_id: "gate_7_b_fix_plan_review"`, `post: "python scripts/update_state.py --command oracle_gate_7b --project-root ."`; if `last_status == "ORACLE_ALL_CLEAR"`, call `complete_oracle_session(state, "all_clear")`, save state, emit `pipeline_complete` with "Oracle all clear" message; if `last_status == "ORACLE_HUMAN_ABORT"`, abandon oracle session, emit `pipeline_complete`; otherwise emit `invoke_agent` with `agent_type: "oracle_agent"`, prepare task prompt with green run context. **(Bug S3-44 fix, Bug S3-82 fix.)**
  - `oracle_phase == "gate_b"`: emit `human_gate` with `gate_id: "gate_7_b_fix_plan_review"`, `post: "python scripts/update_state.py --command oracle_gate_7b --project-root ."`. Present fix plan for human review. Gate 7B is exclusively for `ORACLE_FIX_APPLIED`. **(Bug S3-44 fix, Bug S3-82 fix.)**
...
- `"git_repo_agent"` + `REPO_ASSEMBLY_COMPLETE`: no state change (two-branch routes to gate_5_1).
- `"bug_triage_agent"` + `TRIAGE_COMPLETE: single_unit` / `TRIAGE_COMPLETE: cross_unit`: load `.svp/triage_result.json`, set `debug_session.classification` and `debug_session.affected_units` from triage result **(Bug S3-84 fix)**. Two-branch routes to gate_6_2.
...
- `"oracle_start"`: status_line may contain a sentinel value (e.g., `"ORACLE_REQUESTED"`) written by the command skill. Normalize any non-path value to empty string before passing to `enter_oracle_session()`, so that `_route_oracle()` presents the deterministic test project selection gate. **(Bug S3-81 fix.)**
- `"oracle_test_project_selection"`: status_line contains the selected test project path; set `oracle_test_project` directly.
- `"oracle_gate_7a"`: status_line contains the human's gate response (from `last_status.txt`). Calls `dispatch_gate_response(state, "gate_7_a_trajectory_review", status_line, project_root)` to perform the `oracle_phase` transition. **(Bug S3-82 fix.)**
- `"oracle_gate_7b"`: status_line contains the human's gate response (from `last_status.txt`). Calls `dispatch_gate_response(state, "gate_7_b_fix_plan_review", status_line, project_root)` to perform the `oracle_phase` transition. **(Bug S3-82 fix.)**
...
- Nested session bootstrap: `_bootstrap_oracle_nested_session` creates workspace at green_run start. Mode-aware artifact resolution **(Bug S3-83 fix):** reads `state.oracle_test_project`; if path starts with `"examples/"`, resolves artifacts from `project_root / oracle_test_project` (E-mode: `stakeholder_spec.md` → `specs/`, `blueprint_prose.md` + `blueprint_contracts.md` → `blueprint/`, `project_context.md` → root); otherwise copies from workspace `specs/` and `blueprint/` directories (F-mode, existing behavior). Both modes copy `.svp/` for pipeline state skeleton and `svp_config.json`.
